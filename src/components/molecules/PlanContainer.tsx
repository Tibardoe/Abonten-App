...
export default function PlanContainer({
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className={cn(
        "w-full md:h-36 relative flex flex-col md:grid gap-3 md:grid-cols-[150px_1fr_auto] border-[4px] md:gap-5 md:border-[6px] items-center justify-start border-border",
    ...
    >
      {isActive && (
        <span className="bg-mint rounded-full grid place-items-center absolute top-1/2 -translate-y-1/2 -left-4 h-6 w-6 md:h-9 md:w-9 md:-left-5">
          <div className="border-r-[2.5px] border-b-[2.5px] border-primary-foreground w-[8px] h-4 rotate-45 -translate-y-[2px] md:-translate-y-1 md:border-r-[3px] md:border-b-[3px] md:w-[11px] md:h-5" />
        </span>
      )}

      <span className="w-full h-full p-5 grid place-items-center bg-muted">
