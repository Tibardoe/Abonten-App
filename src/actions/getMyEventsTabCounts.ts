...
/**
 * Cheap counts for the Active/Cancelled/Refunds tab badges — three
 * `head:true` exact counts (index-backed on ticket.user_id/status), never a
 * full row fetch just to size a badge.
 */
export default async function getMyEventsTabCounts(): Promise<{
  ...
  if (userError || !user) {
    return {
      status: 500,
      data: { active: 0, cancelled: 0, refunds: 0 },
      message: "User not logged in",
    };
  }

  const [activeResult, cancelledResult, refundsResult] = await Promise.all([
    supabase
      .from("ticket")
  ...
  if (activeResult.error || cancelledResult.error || refundsResult.error) {
    console.error(
