...
- **Removed**: `finalizePaystackPayment`, `verifyPaystackPaymentCore`, `submitPaystackChargeOtpCore`, `paystackInit`, `gateway/paystackService`, `gateway/paystackTransfer`, `hubtelOtpClient`, `/api/mobile/paystack/momo-networks` (now `/api/mobile/payments/momo-networks`), env `PAYSTACK_TRANSFERS_ENABLED`.
