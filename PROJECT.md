...
| `payment_attempt` | — | **New** (same migration). Separates "an attempt to pay" from the checkout it pays for (`ticket_checkout` session via `checkout_session_id`, or `subscription_checkout` via `subscription_checkout_id` — exactly one required) and the `payment_method` used. Lifecycle `status`: `initiated/pending/processing/succeeded/failed/cancelled/refunded`. No payment gateway sets it to `succeeded` yet (see §16) — it exists so a future gateway integration has a real row to drive instead of mutating checkout status directly. |
| `promo_code` | — | |
...
5. **No payment-gateway integration in this repo, though the database expects one (Flutterwave).** `validateCheckout`/`generateTicket` compute price and issue tickets, but no code path in this repo calls Flutterwave (or any payment API), and no action ever inserts a `transaction` row — see §7.6 discrepancy #2. Tickets can currently be generated without a linked transaction at all, since `ticket.transaction_id` is nullable. **As of 2026-08-17, `/transactions` no longer depends on this being fixed** — it was rebuilt against `ticket_checkout`/`subscription_checkout` (see §5's Transactions entry) rather than waiting on a real gateway integration. The `transaction`/`payment_attempt` tables remain unused, reserved for whenever that integration is actually built.
...
18. **Fixed (2026-08-16): `/wallet` was accidentally coupled to checkout state.** Since commit `94d3742`, `/wallet` and `/wallet/[checkoutId]` both rendered the pending-checkout "basket" component, so a completed purchase (whose `ticket_checkout` rows flip to `status='paid'`) made `/wallet` show "No pending checkouts." — wallet management was, in effect, unreachable without an active balance-due checkout. Fixed by giving the checkout basket its own routes (`/checkout`, `/checkout/[checkoutId]`) and making `/wallet` a real, checkout-independent payment-method management page (see §5, §7.6 discrepancy #6, §7.2's `payment_method`/`payment_attempt` rows). Ticket and subscription checkout now share one `PaymentMethodSelector` component for picking/adding a saved method during payment. **Remaining limitation**: no payment gateway is integrated, so `payment_attempt` rows never advance past `initiated` — paid checkouts show an honest "awaiting payment provider integration" state rather than completing or faking success. This is the same pre-existing gap noted in item 5, now scaffolded with real schema (`payment_attempt`) instead of a dead-end disabled button.
...
- **Resolved 2026-08-16**: `payment_method` is wired up to a real, checkout-independent `/wallet` page (see §5, §7.6 discrepancy #6). Remaining gap: no payment gateway drives `payment_attempt` to `succeeded` yet, so paid ticket/subscription checkouts still can't be completed end-to-end — only the honest "payment attempt created, awaiting provider integration" state exists (see §16).
- **Resolved 2026-08-17**: `/transactions` was rebuilt into an analytics + history page sourced from `ticket_checkout`/`subscription_checkout` instead of the permanently-empty `transaction` table (see §5's Transactions entry, item 5 above). Remaining gaps carried forward, not fixed by this change: still no payment-gateway integration (so `transaction` stays unused), and still no refund tracking (`issueRefund` remains a stub) — a cancelled ticket's originating `ticket_checkout` row stays `status='paid'` and is counted as a successful transaction. No organizer-facing transactions view was added; organizer revenue stays on the separate `/manage/dashboard`.
