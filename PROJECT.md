...
> **Revision note (2026-09-07):** In-app messaging system — **Phases 1–3 done (database + backend transports + realtime layer), work in progress** on branch `feat/in-app-messaging`. A production messaging layer for user ↔ organizer, user ↔ place, and user ↔ support conversations (mobile + web + admin moderation), replacing the "get the organizer's phone number" gap. This note covers only what is live in the database; the transports, realtime, and UI land in later phases and this section will be expanded then. **Schema (migrations `20260907090000_messaging_schema` + `20260907090100_messaging_rpcs` + `20260907090200_messaging_fk_covering_indexes`, all applied live via MCP + `get_advisors` clean):** six new tables — `conversation` (`type` event/place/support/direct, nullable `event_id`/`place_id` FKs `ON DELETE SET NULL` so deleting a subject never destroys the thread, denormalized `last_message_*`, `moderation_state`), `conversation_participant` (`role` member/organizer/place_owner/staff/admin — built for multi-member business inboxes later; per-user `last_read_at`/`muted`/`archived`/`left_at`), `message` (`message_type` text/image/file/system, `content` ≤4000, `reply_to_message_id`, `client_generated_id` for optimistic-send dedupe, soft `deleted_at`, `edited_at`, `moderation_state`), `message_attachment` (metadata for a **private** `message-attachments` Storage bucket — the `place-claim-documents`/`report-attachments` pattern), `message_reaction` (schema only — no UI in V1), `conversation_block` (server-enforced). Partial unique indexes on `(event_id, created_by)` / `(place_id, created_by)` / open support give `open_conversation` a race-safe get-or-create. **Security model — identical to the recent `issue_free_ticket` / `create_ticket_checkout` hardening:** `conversation` / `conversation_participant` / `message` have **no client INSERT/UPDATE/DELETE grants**; every mutation is a `SECURITY DEFINER` RPC keyed on `auth.uid()` — `open_conversation`, `send_message` (participant + block + closed + rate-limit + attachment-prefix checks; idempotent on `client_generated_id`), `edit_message` (own, 15-min window), `delete_message` (own, soft), `mark_conversation_read`, `set_conversation_state` (mute/archive), `block_participant`, plus read RPCs `list_conversations` (keyset, per-conversation unread) and `get_unread_conversation_count` (badge). RLS SELECT on every table is "are you a participant" via a new `SECURITY DEFINER` helper `public.is_conversation_participant(conversation_id, user_id default auth.uid())` (bypasses RLS internally to avoid recursion); staff also read via `is_staff()`. The generic `report` table's `target_type` CHECK was widened with `message` / `conversation`. **Verification so far:** `20260907090100`'s 15 RPCs + 6 tables + storage bucket + 10 policies confirmed present via `execute_sql`; `get_advisors` security = clean, performance flagged 3 FK covering indexes which `20260907090200` adds; `database.types.ts` regenerated; full monorepo `npm run typecheck` green (11/11). **49 integration tests green** against a local Supabase stack (`npm run test:integration -w @abonten/services`), 19 of them new: `messaging.integration.test.ts` (13 — get-or-create idempotency + race, RLS/IDOR read + write denial, `client_generated_id` dedupe, unread tracking, edit-window + author-only edit/delete, block both-directions, RPC-level IDOR, role-escalation denial) and `messaging-service.integration.test.ts` (6 — Core error mapping, message-page shaping incl. reply previews + deleted-message redaction, conversation-context 404-on-non-participant). **Phase 2 (backend transports):** `packages/services/src/messaging/**` — `openConversationCore`, `sendMessageCore` (+ best-effort notification/push fan-out to other non-muted participants via `createNotificationCore`, `kind: "message"`), `editMessageCore` / `deleteMessageCore`, `markConversationReadCore` / `setConversationStateCore` / `blockParticipantCore`, and direct-RLS read queries `fetchConversationsPage` / `getConversationContext` / `getUnreadMessageCount` / `fetchMessagesPage` (keyset, embeds attachments + reply previews, redacts soft-deleted rows). `messagingError.ts` maps the RPC SQLSTATE to the `{status,message}` envelope. 11 web Server Actions in `apps/web/src/actions/` (`openConversation`, `getConversations`, `getConversationDetail`, `getConversationMessages`, `sendMessage`, `editMessage`, `deleteMessage`, `markConversationRead`, `setConversationState`, `blockConversationParticipant`, `getUnreadMessageCount`) + 11 `apps/web/src/app/api/mobile/messages/**` route handlers (open · list · unread-count · `[conversationId]` · `[conversationId]/messages` · send · edit · delete · read · state · block) + a `messaging` namespace on `@abonten/api-client` (`scripts/check-mobile-api-parity.mjs` green, 100 routes). New `@abonten/types/messagingType` + `@abonten/validation/messageSchema`. `next-intl`/mobile notification link routing extended for `message` (`/messages/:id` → `/(app)/messages/:id`). The generic `report` path (`submitReportCore`, `reportSchema`, admin `reportsAdminCore` snapshot map, `@abonten/api-client` `SubmitReportBody`) now accepts `message` / `conversation` targets. `npm run typecheck` (11/11) + `npm run build` (web + admin) green. **Phase 3 (realtime delivery layer):** migration `20260907091000_messaging_realtime` adds `message` / `conversation` / `conversation_participant` to the `supabase_realtime` publication with `REPLICA IDENTITY FULL` (postgres_changes stream — new message, edit, soft-delete, `last_message_*` bump, read-receipt/mute/archive; authorized by the tables' own participant-only RLS), and two `realtime.messages` policies (`messaging_realtime_participant_{read,write}`) that gate the **private** `conversation:<uuid>` broadcast/presence channel — used for the ephemeral typing indicator + who's-in-the-thread presence, **never persisted** — to participants only. Shared contract in `@abonten/core/messagingRealtime` (`conversationChannelName`, `TypingBroadcast` / `ConversationPresenceState` payloads, `TYPING_TTL_MS` / `TYPING_THROTTLE_MS`, `presenceRecency()`), unit-tested. `messaging-realtime.integration.test.ts` proves a participant can join the private channel and a non-participant gets `CHANNEL_ERROR` (stable across repeated runs); 50 integration tests green. The client-side subscription/reconnect/typing-UI wiring lands with each platform (Phase 4 mobile, Phase 5 web); the postgres_changes *delivery* round-trip is verified there on a real client rather than via a WAL-timing assertion in CI. **Not built yet: Phase 4 mobile (Wallet→Messages nav swap + all UI), Phase 5 web UI, Phase 6 organizer/place inbox, Phase 7 push wiring + badges, Phase 8 block/report UI + admin moderation viewer.**
