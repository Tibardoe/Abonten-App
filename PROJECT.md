...
>
> **Verification:** `npm run typecheck` 11/11, `npm run build` (web + admin) green, `check-mobile-api-parity.mjs` green, Biome clean. All 8 migrations applied live to `sderrexhawjbmsugndcq`, advisor security clean (the messaging RPCs' "SECURITY DEFINER executable by authenticated" notices are this repo's accepted pattern), and `execute_sql`-confirmed. Integration tests: the pre-existing 50 messaging tests (`messaging` / `messaging-service` / `messaging-realtime`) plus a new `messaging-moderation.integration.test.ts` (role-scope inbox split; hidden conversation leaves both inboxes; hidden/removed message leaves the thread) — **need the local Supabase stack rebuilt for the Phase 3–8 migrations, which is currently blocked because Docker Desktop isn't running** (not a code issue): `npm run test:db:down && npm run test:db:up && npm run test:integration -w @abonten/services`. **Not device-verified:** the rendered mobile/web UI, realtime round-trips on real clients, image pick/upload, keyboard behaviour, push delivery — a manual device/browser pass is the remaining gate.
