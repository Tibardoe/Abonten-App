...
>
> **Realtime** (`20260907091000`, redesigned `20260918120100` — §35.8). **One transport: Broadcast on private channels.** `conversation:<uuid>` (participant-only via two `realtime.messages` policies) carries the client-sent "typing" broadcast and presence, plus the durable change events database triggers send when a write commits (`message_insert`, `message_update`, `reaction`, `participant_update` — ids only, never message bodies). `inbox:<uid>` (readable only by that user, policy `messaging_realtime_inbox_read`) carries `conversation_update` (last-message fields), `conversation_added` / `_removed` / `_state`. Each private topic has exactly one owner per app (`openPrivateChannel` in `@abonten/core/messagingRealtime` removes any stale copy before joining — supabase-js hands back an existing channel for a repeated topic). Postgres is the source of truth: an event triggers a React Query refetch through RLS or a patch of a cached inbox row; the socket auto-reconnects and each fresh `SUBSCRIBED` reconciles. Until 2026-09-18 the durable half rode `postgres_changes` on a non-private `msgchanges:<uuid>` channel; the four tables stay in the `supabase_realtime` publication only until every client has updated (§35.8). Typing + presence are **ephemeral, never persisted**.
>
...
- **`ticket_type`**: `id`, `event_id` (FK CASCADE), `type` (free text — organizer-defined, no CHECK by design), `price` (numeric, CHECK `>= 0` since 2026-09-04), `quantity` (integer, CHECK `>= 0` since 2026-09-04), `available_from`, `available_until`, `currency` (free text), `created_at`.
...
- **`promo_code`**: `id` (default `gen_random_uuid()`), `event_id` (FK CASCADE, nullable), `promo_code` (text, **nullable**, unique), `discount_percentage` (integer, CHECK 0–100 since 2026-09-04), `expires_at`, `max_uses`, `times_used` (default 0), `is_active` (default true — a static flag; the DB has no trigger to flip it automatically when `expires_at` passes), `created_at`.
...
4. **Resolved 2026-09-04 (recorded 2026-09-18) — was: `ticket_code` has no uniqueness guarantee at the database level.** Migration `20260904131325_phase2_ticketing_constraints.sql` added the partial unique index `ticket_ticket_code_key` on `ticket (ticket_code) WHERE ticket_code IS NOT NULL`; confirmed live on 2026-09-18 (`pg_indexes`). Application code still generates the code, but a collision now fails the insert instead of going unnoticed.
5. **Resolved 2026-09-04 (recorded 2026-09-18) — was: ticket type price/quantity and promo discount unconstrained in the database.** The same migration added `CHECK (price >= 0)` and `CHECK (quantity >= 0)` on `ticket_type` and `CHECK (discount_percentage IS NULL OR discount_percentage BETWEEN 0 AND 100)` on `promo_code`. A second, logically identical copy of each check had been added under a different name; the duplicates were dropped on 2026-09-18 (`20260918100000_audit_fk_indexes_and_constraint_dedupe.sql`). `ticket_type.type` remains free text by design (organizer-defined categories).
...
13b. **Resolved 2026-09-04 (recorded 2026-09-18)** — was: `ticket.ticket_code` not unique, no `ticket_type` price/quantity checks, no `promo_code.discount_percentage` range check. All three now exist in the database (partial unique index `ticket_ticket_code_key`, `ticket_type_price_nonnegative`, `ticket_type_quantity_nonnegative`, `promo_code_discount_percentage_range`); see §7.6 discrepancies #4–#5. `ticket_type.type` stays free text by design.
...
- **Resolved 2026-09-04**: `ticket.ticket_code` has a partial unique index and `ticket_type.price`/`quantity` and `promo_code.discount_percentage` have `CHECK` constraints (see §16 item 13b). `ticket_type.type` is intentionally free text.
...
- **admin hardening** (`apps/admin/src/lib/sentry.ts`, 2026-09-04) — admin's Sentry project is its
  *only* monitoring sink, so the factory adds:
  ...
  - **swallowed Server Action failures** — actions catch their throw and return an envelope, so
    `onRequestError` never sees them; `server/actions/_shared.ts` (`adminError`, since §37) routes non-expected errors through a
...
- Schemas `adminRefundSchema` / `settlePayoutSchema` / `createPayoutSchema`; actions
  `refundTransaction` / `settlePayout` / `createPayout` in `apps/admin/src/server/actions/finance.ts` (one file per domain since §37).
...
### 23.15 Admin metrics, definitions and clarity — 2026-09-13

A production-grade audit of every number the console shows, then six phases of fixes. Read-only production SQL and a page-by-page trace found the money tiles disagreeing with the ledgers and with each other, and nothing on any page explaining a figure, a period or a status.

**What was wrong (fixed in phase 0, commit `35a47317`):** organizer earnings omitted `refund_adjustment` rows and "outstanding" was clamped at zero; the dashboard summed the whole charge (fee included) of refunded *and merely requested* refunds by `updated_at`; "gross ticket sales" summed both `platform_fee_entry` types and was therefore net of refunds (production showed GH₵240 where sales were GH₵1,264) while Analytics' "gross" included fee and credit; net revenue read a missing Paystack cost as zero; "organizers" and "tickets" each had three definitions; dates rendered in the server's locale; Finance still said refunds and payouts were "a later phase".

**Shared vocabulary (`@abonten/core/admin/`):** `metricDefinitions.ts` (label, definition, period, source, caveats per `MetricKey`; `docs/admin/metrics.md` is generated by `npm run docs:metrics` and a test fails on drift), `statusLabels.ts` (every enum family → label + tone + icon, readable fallback), `adminDateRange.ts` (`AdminRangeKey` today/7d/30d/90d/ytd/custom → half-open calendar-day windows in Africa/Accra with the equivalent previous window; `parseAdminRangeParams` for URLs), `computeTrend.ts` (moved from the organizer dashboard so both apps agree), `smallSample.ts` (5-person floor plus complement suppression), `describeSeries.ts` (chart summary for screen readers).

**UI kit (`apps/admin/src/components/metrics/`):** `MetricCard` (six states: ok / zero / no-data / insufficient / unavailable / error, period line, trend, secondary line, drill-down), `Trend`, `InfoTip` (dependency-free toggle-tip: hover, focus, tap, Escape, outside click, flips at the viewport edge), `StatusBadge`, `SectionHeading`, `RangePicker` + `RangeCaption` (links + a GET form, no client JS), `ChartCard` (unit, caption, legend, `role="img"` summary, sr-only table, empty/insufficient states), `charts/TimeSeriesChart` (recharts, mount-gated, previous period dashed), `charts/BreakdownBars`, `Skeleton` family and `ListLoading`; route `loading.tsx` on every list page and `(console)/error.tsx`. Tokens `--popover*` and `--chart-1..5` added to the admin theme. Dark mode stays unwired.

**SQL (service_role only, `security definer`, `search_path=''`):** `admin_dashboard_kpis(from,to,prev_from,prev_to)` (`20260913201544`, also fixes `admin_dashboard_counts.pendingPayouts`, adds eight indexes and the `purge-health-check-result` cron), `admin_platform_analytics(from,to,bucket,prev_from,prev_to)` and `admin_user_demographics(...)` (`20260913204713`), `admin_finance_overview(...)` and `admin_organizer_balance(organizer_id|null)` (`20260913211804`, corrected by `20260913213203` so "paid out" is the payout ledger). Definitions: a sale is a `fee` row and refunds are the `fee_refund_adjustment` mirror, never netted into gross; a paid ticket has a transaction and is not cancelled; an organizer has a non-draft event; net revenue counts only rows with a known cost. `admin_organizer_balance` mirrors `get_organizer_finance_overview()` (`packages/services/src/__integration__/admin-finance-overview.integration.test.ts` asserts parity under the organizer's JWT).

**Demographics, honestly:** `user_info` has no age, gender, location, language or last-active column, and `address` holds only `full_address`. Analytics therefore shows only roles, sign-in method (`auth.users`), mobile platform (push-enabled users, stated as such), account status, buyers / repeat / conversion / returning — aggregate, suppressed below five people for the person-describing breakdowns — and says what is not collected.

**Verification:** 42 new unit tests; 4 new integration files (finance parity, dashboard definitions, analytics zero-fill and suppression, RPC authorisation); full suite 50 files / 449 tests on a clean replay; production RPCs read back via MCP; every rebuilt page rendered end to end against a local stack signed in as a throwaway staff account (this caught a server→client function prop before it shipped).

**Follow-ups, same day (the deferred list closed):** `admin_transaction_refundable_amounts(uuid[])` (`20260913221216`) batches the refunds list's per-row RPC into one call with the arithmetic untouched; `admin_fieldops_commission_totals(campaign_id|null)` (`20260913221225`) sums the field commission ledger in SQL per currency — the overview had been summing rows in the browser under PostgREST's silent 1,000-row default and taking the first row's currency. Rewards › Referrals / Promoters & loyalty / Rebates moved onto the shared period control and the metric registry (`docs/admin/metrics.md` now has Rewards, Discovery and Field Ops sections); their JavaScript summaries keep a row cap but the cores report `truncated: {fetched, total}` (`packages/services/src/admin/shared/capped.ts`) and the pages show an "Incomplete figures" notice (`CapNotice`) instead of a partial sum. Discovery reads its breakdown keys through new `statusLabels` families (search mode, platform, subscription kind/source, suppress and skip reasons, delivery status) and says its window is rolling and uncompared; Field Ops › Campaign figures draws real charts and uses one word, "succeeded", for a passed onboarding. Dark mode is wired (`next-themes`, `ThemeProvider` + `ThemeToggle`, `color-scheme` on both palettes). `InfoTip` was driven in a real browser (Playwright on a local stack: Tab, Escape with focus return, click to pin, outside click) and the console was rendered against production data through the local dev server once `apps/admin/.env.local` carried current keys. `discovery-personalization.integration.test.ts` pins the generator watermark before its second run, so it passes on a re-used stack. Integration test `admin-batched-reads.integration.test.ts` asserts the two RPCs' parity with the per-row functions and refuses signed-in callers.

**Still deferred:** no region mapping of events; `test:db:up` needs `SUPABASE_TEST_PORT_OFFSET` where Hyper-V reserves the default ports.
...
**§30.10 Device pass (2026-09-12, Android emulator, against production).** M2 closed. A local debug dev client was built (`npx expo run:android`) because the installed APK was a pre-verification EAS build serving its own embedded bundle — the deep link never reached Metro, so nothing on the device was running current code. The programme was switched on for `audience = 'staff'` (the only active `admin_user` is the founder, so nothing was exposed publicly) and the whole lifecycle was driven on the device: organizer request started with a type picked → PDF added through the document picker → submitted; place request started → photo added from the gallery → submitted; the admin console (production) showed both in the queue, served the evidence over a 5-minute signed URL (200 `image/jpeg`, real bytes) while the unsigned and public paths both returned 400; "ask for more" with a reason came back to the device word for word as **We need more information** with the CTA changed to **Send again for review**; a second document was added and resubmitted; both cases were approved; the badge and its explanation appeared. **Two gaps were found and fixed in the same pass** — the mobile Verified pill was a plain `View` with no way to learn what the badge means (web has `VerifiedBadgePopover`), and there was **no organizer badge on mobile at all**, so organizer verification produced nothing a mobile user could see. New `apps/mobile/src/components/verification/VerifiedPill.tsx` renders both badges and opens `BADGE_EXPLANATION` on tap, wired into the place hero, the event organizer card (`useEventDetail` now selects `organizer_verified, status_id`) and the profile header (`usePublicProfile` reads those two columns from `user_info`, since `user_profile_details` predates them). **Still not carried over from web:** the per-place and organizer-nav status chips (`VERIFICATION_CHIP_LABEL` is unused on mobile outside the verification screen) and a mobile place-setup checklist; both are cosmetic and need an API call per subject.

---

## 31. Discovery — unified search, `@organizer` search and opt-in recommendation notices (2026-09-13)

Design and ranking: [docs/architecture/discovery-search-and-recommendations.md](docs/architecture/discovery-search-and-recommendations.md). Operator handbook: [docs/admin/discovery.md](docs/admin/discovery.md). Measurements: [docs/architecture/perf/discovery-2026-09.md](docs/architecture/perf/discovery-2026-09.md). Branch `feat/discovery-search-and-recommendations`.

**What it is.** One ranked search across events, places and organizers with an `@handle` mode; a private "Notify me" on organizers and places (no follower counts); explicit opt-in prompts after a ticket, an RSVP, a favorite, a review or a second check-in; capped recommendation digests (push + in-app only, never email); a For you list; Settings › Notifications on web and mobile; Admin › Discovery. **Ships switched off**: `discovery_program_setting.search_v2_enabled = false`, `recommendations_enabled = false`, `recommendations_shadow_mode = true`, `prompts_enabled = false`, audiences `staff`, plus `SEARCH_V2_KILL_SWITCH` / `RECOMMENDATIONS_KILL_SWITCH` web env flags. Resolver: `packages/services/src/search/discoveryProgram.ts` (fails closed, 15 s cache).

**§31.1 Database** (ten migrations `20260913090000`–`20260913090900`, applied to production via the Supabase MCP; function bodies hash-identical to the local replay). Generated `search_tsv` (`simple` config) on `event`, `place`, `user_info` with partial GIN indexes matching the visibility rule, trigram indexes, a `lower(username)` prefix index. `_search_*_pool` candidate functions (precise match first, trigram fallback only under five hits, ORs as UNIONs so each branch uses its index) feed `search_events` / `search_places` / `search_organizers` / `search_suggest` (anon-executable `SECURITY DEFINER`, public columns only, keyset paging on score with pinned `as_of`). `search_query_log` (no user/device/IP; 90-day purge). `notification_subscription`, `notification_prompt_state` (owner SELECT only), new `notification_preference` columns (`recommendations_push`, `organizer_alerts_push`, `place_updates_push`, `social_push`, `paused_until`; `*_email` columns exist, unused). `event/place.published_at` + trigger; `recommendation`, `recommendation_digest`, `recommendation_digest_skip` (service-role only); `recommendations_generate`, `recommendations_build_digest`, `recommendation_dismiss`, `recommendations_for_user`, `recommendations_purge`, `admin_search_insights`, `admin_recommendation_metrics`. `notification_delivery.source` gains `recommendations` (never urgent); `notification_delivery_claim` skips them as `channel_off` / `opted_out` at send time. Crons: `search-log-purge` 03:35, `recommendations-generate` */15, `recommendations-digest` */10 (acts in the 18:00 Accra hour, resumes where the last run stopped), `recommendations-purge` 03:40. Permissions `discovery.view` (operations, analyst), `discovery.configure` (operations, step-up).

**§31.2 Code.** Services: `packages/services/src/search/{searchCore,discoveryProgram}.ts`, `packages/services/src/notifications/{preferencesCore,subscriptionCore,promptCore,recommendationsCore}.ts`, `packages/services/src/admin/discovery/discoveryAdminCore.ts`; `createNotificationCore` respects `social_push` for messages, reviews, replies and booking updates (`packages/core/src/notifications/categories.ts`). Core: `packages/core/src/search/parseSearchQuery.ts`. Validation: `packages/validation/src/discoverySchemas.ts`. Web: Server Actions in `apps/web/src/actions/discovery/`, `/search` (unified when on, redirects to the old slug search when off), `/for-you`, `/settings/notifications`, `apps/web/src/discovery/**`. Mobile API: `/api/mobile/search`, `search/click`, `discovery/program`, `notifications/{preferences,subscriptions,subscriptions/status,subscriptions/[id],prompt,prompt/shown,prompt/respond}`, `recommendations{,/dismiss,/opened}` (api-client namespaces `search`, `discovery`, `alerts`, `recommendations`). Mobile: `UnifiedSearch` (type-ahead and submitted searches both use the API since 2026-09-15, §31.6), alerts components, `/(app)/settings/notifications`, `/(app)/for-you`, push taps mark the notice read. Admin: `apps/admin/src/app/(console)/discovery/`.

**§31.3 Pre-existing bugs fixed on the way.** Production cron `refresh_search` failed every 15 minutes (the `event_search` matview never existed; five docs described it). `get_event_suggestions` / `get_place_suggestions` returned hidden, removed and archived rows. `/search` multiplied distance by 1,000 and dropped free events whenever the price filter was at "Any". `address->>'name'` never matched (the key is `full_address`). A duplicate `place_visit` index (advisor). The digest job as first written would have starved everyone after the first 5,000 people.

**§31.4 Verification performed.** Unit: core 305, services 67. Integration: 42 files / 384 tests on the replayed local stack, including 26 Discovery cases (ranking, new-vs-popular, visibility, `@handle` incl. display-name word starts, hostile input, paging, logging without identifiers, programme switches, prompt rules, subscriptions, generate/suppress/digest/caps/shadow/opt-out at send time/batched digests/dismissal pause, social push gate). Monorepo typecheck, `check:api-parity` (176 routes), `check:docs`, scoped Biome, `next build` for web and admin (no warnings). Performance harness `scripts/perf/` on 100k events / 20k places / 50k accounts: result pages p95 ≤ 31 ms, suggestions p95 ≤ 56 ms except a misspelling of a very common word at 68 ms; generate 132,844 picks in 4.4 s; digest for 10,000 people 11.2 s. **End-to-end**: web and admin driven in a real browser against a local stack (search, `@` mode, organizer bell, RSVP prompt, place prompt and "Not now", preference centre, pause/resume, engine run → bell → For you → dismiss, admin overview, settings save with step-up and audit, step-up required); Android emulator against the same stack (type-ahead, results tabs, `@` search, email-code sign-in, preference centre toggles and Stop, organizer bell, inbox → For you, free RSVP → prompt → accept). E2E found and fixed: every web search returning 400 (null filter list) shown as "No results"; RSVP and favorite prompts never appearing (mutation callbacks read the optimistic state); alerts hidden after sign-in (programme cache not keyed by user, web and mobile); an admin hydration error; a misleading "Not interested" rate. **Not verified**: the paid-ticket prompt (needs a real Paystack payment), iOS, real push delivery of a recommendation digest to a device, production latency.

**§31.6 Limitation fixes (2026-09-15).** Four migrations `20260915100000`–`20260915100300`. (1) **Place services searchable**: generated `place_service.search_tsv` + GIN; `_search_place_pool` gains a services branch (ranked below name and category matches) — result pages and type-ahead both use it. (2) **Type-ahead rate-limited and logged**: mobile type-ahead now goes through `GET /api/mobile/search/suggest` (180/min, programme switches) instead of the anon RPC (the earlier hybrid decision reversed by the owner); both platforms log `surface = 'suggest'` rows and attribute the opened suggestion; `admin_search_insights` adds a separate `suggestions` block; Admin › Discovery shows three type-ahead tiles. Pre-change app builds still call the RPC until they update. (3) **Expo push receipts**: `push_receipt` holds accepted tickets; `run_notification_delivery` wakes `/api/notifications/deliver` when one is due; `pushReceiptsCore` prunes `DeviceNotRegistered` tokens and logs provider errors. (4) **Web push**: `web_push_subscription` (push-service host allowlist, ≤10 per person), `webPushCore` + `web-push` dependency, `apps/web/public/push-sw.js`, `useWebPush` + Settings › Notifications › Browser notifications, `/notifications/open` click landing (marks read, same-site redirect), sign-out releases the browser, account deletion removes subscriptions; needs `WEB_PUSH_VAPID_PUBLIC_KEY` / `WEB_PUSH_VAPID_PRIVATE_KEY` / `WEB_PUSH_SUBJECT`. (5) **Recommendation email, locked off (legal G1)**: `discovery_program_setting.recommendations_email_enabled` (false), `notification_consent_event`, per-channel reason switches, email queued beside the push by `recommendations_build_digest`, channel-aware claim, two-channel outcome trigger, `recommendation_digest_email_items()`, `RecommendationDigestEmailTemplate` + `sendRecommendationDigestEmail`, `/unsubscribe/recommendations` + one-click `topic=recommendations`, opt-in switch on web and mobile shown only when allowed, admin toggle that requires `confirmLegalG1`, `RECOMMENDATION_EMAIL_KILL_SWITCH`. Verified: integration suite 53 files (new `discovery-limitations` file, 12 cases), perf harness with 100,000 services. Not verified: real browser push services, a real recommendation email through Resend, the mobile type-ahead route on a device (needs an EAS build).

**§31.5 Open.** Rollout is the founder's call (N1); caps, prompt frequency and retention are working defaults (N2–N4); legal **G3** (promotional push consent, keeping the opt-in record) before shadow mode is turned off for anyone outside staff. Mobile changes reach devices only with a new EAS build or update.

## 32. Abonten Weekly — editorial weekly editions of events and places (2026-09-13)

Design: [docs/architecture/weekly-highlights.md](docs/architecture/weekly-highlights.md). Operator handbook: [docs/admin/weekly.md](docs/admin/weekly.md). Branch `feat/weekly-highlights`.

**What it is.** A weekly edition of events and places worth discovering, for Ghana and later for regional areas, curated by staff in Admin › Abonten Weekly and shown at `/weekly`, in an Explore teaser and in the app. Editions reference live listings; nothing is copied. **Ships switched off**: `weekly_program_setting.enabled = false`, audience `staff`, plus the `WEEKLY_KILL_SWITCH` web env flag. Resolver: `packages/services/src/weekly/weeklyProgram.ts` (fails closed, 15 s cache).

**§32.1 Database** (migration `20260913120000_weekly_core.sql`, applied to production via the Supabase MCP as `20260913135624 weekly_core`; all 16 function bodies hash-identical to the local replay; security advisors show only the expected "RLS enabled, no policy" notices for the five new tables). Tables `weekly_program_setting`, `weekly_scope` (Ghana seeded; one national scope per country; reserved slugs), `weekly_edition` (`unique (scope_id, week_start)`, Monday check, `version`), `weekly_section` and `weekly_item` (deferred unique positions). Functions (all `SECURITY DEFINER`, `search_path = ''`, service role only): `weekly_subject_validity`, `weekly_edition_document` (two set-based reads, public form strips internal fields), `weekly_edition_view` (exact week, or this scope → Ghana → last week's scope → last week's Ghana, with flags), `weekly_resolve_scope`, `weekly_claim_edit`, `weekly_edition_create` (with copy), `weekly_sections_reorder`, `weekly_items_reorder`, `weekly_item_move`, `weekly_edition_validation`, `weekly_edition_transition` (writes `admin_audit_log`), `weekly_publish_due` (opens an `incident`, component `weekly`, on failure), `weekly_housekeeping`, `weekly_health`. Crons `weekly-publish-due` */5 and `weekly-housekeeping` 02:45. Permissions `weekly.view`, `weekly.edit`, `weekly.publish`, `weekly.configure` (last two step-up).

**§32.2 Code.** Shared: `packages/types/src/weeklyType.ts`, `packages/core/src/weekly/` (week maths, section kinds, icons, editorial text sanitising, copy), `packages/validation/src/weeklySchemas.ts`. Services: `packages/services/src/weekly/` (programme, document mapping, edition and teaser reads, signed 30-minute preview tokens) and `packages/services/src/admin/weekly/` (editions, sections, listings, lifecycle, listing picker, preview link, settings, areas). Admin: `apps/admin/src/app/(console)/weekly/` and Server Actions in `apps/admin/src/server/actions.ts`; health probe in `runHealthChecksCore.ts`. Web: routes in `apps/web/src/app/(pages)/weekly/`, components in `apps/web/src/weekly/`, actions in `apps/web/src/actions/weekly/`, Explore teaser, sidebar link, `/weekly` on the middleware public allowlist, mobile API `/api/mobile/weekly`, `weekly/teaser`, `weekly/program` (api-client namespace `weekly`). Mobile: `app/(app)/weekly/`, `src/components/weekly/`, `src/features/weekly/useWeekly.ts`, Explore teaser, drawer row, deep links in `app/+native-intent.ts` and the `/weekly` App Link prefix in `app.json`.

**§32.3 Verification performed.** Unit: core 334, services 77. Integration: 44 files / 416 tests on the replayed local stack, including 32 Abonten Weekly cases (client access refused, programme and audience gates, fallback order, validity drops, public field allow-list, lifecycle, concurrency conflicts, reorders that do not burn a version, scheduler and incidents, housekeeping). Monorepo typecheck, `check:api-parity` (179 routes), `check:docs`, scoped Biome, `next build` for web and admin. **End-to-end** against a local stack: admin in a real browser (create, editorial note, picker by name, link and code, headline, reorder, conflict with reload, preview link and a tampered link refused, publish, settings with step-up, audit rows); web (staff client path, non-staff fallback, anonymous HTML carrying no edition and `noindex`, audience Everyone server-rendered with canonical and Open Graph tags, cache MISS then HIT, 404 for unknown areas and weeks, Explore teaser, 390 px layout, a cancelled event leaving a published edition, mobile API status codes and cache headers); Android emulator (teaser, edition screen, place navigation, unknown-area deep link, drawer row). **Not verified**: iOS; the `/weekly` Android App Link (needs a native build); production pages (the programme is off).

**§32.4 Deliberate limits.** Editions are manual: no suggestions, personalisation, weekly notice or engagement analytics yet (designed in the architecture doc §11). No archive pages. English copy only. Admin reordering uses buttons, not drag and drop.

**§32.5 Open.** Rollout and areas are the founder's call (K1, K2); warning thresholds and retention are working defaults (K3, K4). Mobile changes reach devices only with a new EAS build or update. Noticed on the way and not changed: `/events/[eventCode]` and `/places/[slug]` set `revalidate = 60` without `generateStaticParams`, so Next.js renders them on every request.

## 33. Holistic platform audit — hardening pass (2026-09-13)

Report: [docs/audit/03-holistic-audit-2026-09-13.md](docs/audit/03-holistic-audit-2026-09-13.md). Branch `feat/holistic-audit-hardening`. Four migrations, all applied to production via the Supabase MCP the same day and replayed by the integration suite.

**§33.1 Account deletion preserves the financial record** (`20260913200100`). The live foreign keys cascade from `auth.users → user_info` into `transaction`, `ticket`, `payment_attempt`, `organizer_ledger_entry`, `payout`, `payout_account`, `platform_fee_entry` (via `transaction`) and `event` (with every attendee's ticket). The old `deleteAccountCore` hard-deleted the Auth user, so one "Delete account" tap erased all of that. Now: `account_deletion_blockers()` refuses (409) while the person has upcoming events with attendees, a payout in `processing`, unpaid earnings or admin roles; `credit_close_account()`; `anonymize_deleted_account()` (profile → "Deleted user" / `deleted_<id>`, status 4, personal rows deleted, cards and payout accounts scrubbed to shells because attempts/payouts reference them, places unclaimed, empty upcoming events cancelled, other events archived, verification cases withdrawn/revoked, pending claims removed with their documents queued for purge); then `auth.admin.deleteUser(id, shouldSoftDelete = true)` — sessions, identities, email and phone go, the row stays, nothing cascades. `user_status` 4 = Deleted; proxy, `getMobileAuth` and the admin Users module treat it. Web now signs out and leaves the page after a successful deletion.

**§33.2 Storage purge queue** (`20260913200000`). `purge-reviewed-claim-documents` and `purge-verification-evidence` deleted from `storage.objects` directly; Supabase's `storage.protect_delete()` refuses that (`cron.job_run_details` showed both failing on every run since they were scheduled), and even before the guard it never removed the underlying files. Retention SQL now enqueues `(bucket, path)` into `storage_purge_queue`; the `storage-purge-dispatch` cron (*/10) pokes `POST /api/maintenance/storage-purge` (token in `storage_purge_config`, same pattern as notification delivery), and `@abonten/services/platform/storagePurgeCore` deletes through the Storage API with claim/finish semantics (8 attempts → `failed`). Account deletion uses the same queue.

**§33.3 Refund request race** (`20260913200200`). `issueRefundCore` asked Paystack for the (partial) refund before `record_refund_hold` moved the transaction to `refund_pending`, so two concurrent callers could both pass the status check and both request a refund — and Paystack accepts several partial refunds up to the charge. `claim_transaction_refund()` (service role, CAS on the new `refund_claimed_at` column; released on failure, expires after 2 minutes; migration `20260913200300`) now gates the Paystack call; `cancelUserTicketCore` flips the ticket with a CAS so a double tap cannot release a seat twice.

**§33.4 Day-before event reminders** (`20260913200200`). `event_reminders_enqueue()` (hourly at :07) writes `event_reminder` notices for active ticket holders of a session 23–25 h away, once per person per session (`event_reminder_sent`), skipping accounts that are not active and people with their own app reminder (`event_reminder`); the app-push trigger now includes the type, so it also goes out through the delivery queue (non-urgent: waits out the night). Mobile routes it to the ticket (`data.kind = 'ticket'`).

**§33.5 Web hardening and SEO.** `X-Frame-Options: DENY`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`, HSTS on web and admin (no CSP yet — Paystack inline + Maps need an allow-list). `/robots.txt` (keeps crawlers out of everything signed-in), `/sitemap.xml` (published events ending within 30 days, published places, static pages; hourly), schema.org `Event` / `LocalBusiness` JSON-LD on the listing pages, `metadataBase`. Both metadata routes and `/api/maintenance` are excluded from the session proxy.

**§33.6 Verification.** Unit: services 83 / core 339. Integration: three new suites (account deletion, refund claim + reminders, storage purge; 14 tests) and the full suite on the replayed stack (47 files, 430 tests, all passing). Typecheck for services, web and admin; scoped Biome; `check:docs`; `check:api-parity` (179 routes); `next build` for web. Production: migrations applied, cron jobs present, the two purge functions run clean by hand. **Not verified**: a real deletion in production, the storage purge route end to end against a real bucket object (unit-level with a stubbed Storage call only), the reminder push on a device, iOS anything.

**§33.7 Deferred (in the report).** Content-Security-Policy; Postgres minor upgrade and leaked-password protection (dashboard actions); Cloudinary purge for deleted accounts; product analytics; web push; supply-side growth work; unindexed FK indexes.

## 34. Spotlight & Stories — short video, 24-hour Stories, follows and promoted Spotlights (2026-09-16)

Design: [docs/architecture/spotlight-and-stories.md](docs/architecture/spotlight-and-stories.md). Operator handbook: [docs/admin/spotlight.md](docs/admin/spotlight.md). Pre-implementation audit: [docs/audit/spotlight-stories-pre-implementation-report.md](docs/audit/spotlight-stories-pre-implementation-report.md). Branch `feat/spotlight-stories-platform`.

**What it is.** Spotlight: a ranked feed of short vertical videos and photos (For you, Following, Nearby, Happening soon, Trending) posted by organizers, place owners and staff, each able to link an event or place. Stories: posts that expire after `story_ttl_hours` (24), shown as a row at the top of Messages. Follow: a public follow of an organizer or place (separate from the private alert bell). Promoted Spotlights: fixed-price plans, paid in cash through Paystack, reviewed before they run, labelled "Sponsored". **Ships switched off**: `content_program_setting` Spotlight, Stories and promotions false, audiences `staff`, plus `SPOTLIGHT_KILL_SWITCH` / `STORIES_KILL_SWITCH`. Resolver `packages/services/src/content/contentProgram.ts` (fails closed, 15 s cache).

**§34.1 Database** (migrations `20260916120000_content_platform_core`, `…120100_feed_and_engagement`, `…120200_campaigns`, `…120300_function_grants`, `…120400_moderation_targets`, `…120500_content_campaign_resume_guard`; all applied to production via the Supabase MCP; advisors show only the intentionally public `follow_counts` and the usual RLS-no-policy notices). One `content_post` table for both kinds with `content_media`; engagement, telemetry and rollup tables; campaign tables with an append-only ledger. No client write grants anywhere; RLS lets clients read live posts and their own rows. Key functions (service role only, `SECURITY DEFINER`, `search_path = ''`): `content_post_is_public`, `content_publisher_eligible`, `content_post_publish`, `content_post_documents`, `content_feed`, `content_sponsored_candidates`, `content_story_tray`, `content_view_ingest`, `content_click_ingest`, `content_rollup_stats`, `content_trending_refresh`, `search_spotlight`, `content_housekeeping`, `content_campaign_transition`, `content_campaign_activate_from_checkout`, `content_campaign_accrue`, `content_campaign_refundable_minor`, `content_campaign_record_refund`, `content_campaign_tick`, `content_attribute_conversions`, `content_campaign_reconcile`, `content_admin_overview`, `content_post_insights`. `report` and `moderation_action` accept `spotlight`, `story`, `content_comment`; `payment_attempt.content_campaign_checkout_id` is the fifth payment target. Seven cron jobs (see `docs/operations/scheduled-jobs.md`). Permissions `spotlight.view`, `spotlight.campaigns.review` (step-up), `spotlight.configure` (step-up).

**§34.2 Code.** Shared: `packages/types/src/contentType.ts`, `packages/core/src/content/` (copy, reactions, story expiry, hashtags, sponsored merge, campaign state machine and money, view tracking, limits, links), `packages/validation/src/contentSchemas.ts`. Services: `packages/services/src/content/` (programme, media via the Cloudinary Admin API, posts, feed, stories, engagement, notifications, telemetry, follows, attachable events, campaigns and the `activateContentCampaign` fulfilment step) and `packages/services/src/admin/content/`. Web: Server Actions `apps/web/src/actions/content/`, `/api/mobile/content/**` and `/api/mobile/checkout/spotlight-promotion-attempt`, UI `apps/web/src/spotlight/`, routes `/spotlight`, `/spotlight/[postId]`, `/stories/[postId]` (public in the session middleware), `/manage/spotlight/**`, `/user/[username]/spotlight`, checkout type `spotlight-promotion`, Stories row in `ConversationList`, Follow on profiles and places, nav links gated by the programme. Admin: `apps/admin/src/app/(console)/spotlight/` (overview, posts, post detail, comments, promotions, promotion detail with review and refund, settings). Mobile: `app/(app)/spotlight/` (feed with one shared player, post, composer, manage, post insights, promote, campaign), `app/(app)/story/[id].tsx`, `src/components/content/`, `src/features/content/`, Stories row in the Messages tab, drawer rows, Follow and Spotlight strip on profiles and places, `PromotionPaymentSection kind="spotlight"`, deep links (`+native-intent.ts`, `app.json` App Links, web `apple-app-site-association`), notification routing for kinds `spotlight`, `story`, `follow`, `content_campaign`.

**§34.3 Verification performed.** New integration suite `content-platform.integration.test.ts` (16 tests). Full integration regression on the replayed stack: 53 files, 483 tests passing. Unit: core 434, services 115. Typecheck: services, web, admin, mobile. `check:api-parity` (214 routes), `check:docs`, scoped Biome. The suite found and fixed: the feed sent `undefined` RPC arguments (no feed could load); `moderation_action` refused the new targets and also `message` / `conversation` (staff moderation of messages had been failing in production); an advertiser could resume a staff-paused promotion, or resume on a hidden post. A browser run of web and admin against the local stack (feed, Stories, comments, creator hub, promote, checkout, cancel, admin review and approval) found and fixed four more: the campaign checkout embed was ambiguous; **promotion checkout cancel failed for event, place and Spotlight promotions** (client update rights were removed by the 2026-09-10 lockdown — `cancelPromotionCheckout` now writes with the service role after the owner check); the Story ring refreshed before the last view was sent; the feed pane overflowed. **Not verified**: the mobile app on a device; real Cloudinary uploads and renditions; a sponsored slot on screen (rules unit-tested); a real Paystack promotion payment and refund; iOS; Android App Links for the new paths (need a native build).

**§34.4 Deliberate limits.** No hashtag pages, audio library, duets, live streaming, creator payouts, auction pricing, interest targeting or automated classification. Comments one level deep. Unused promotion budget (cancelled or under-delivered) is refunded by staff, not automatically (decision S3).

**§34.5 Pre-merge audit and the reach-based promotion model (2026-09-16).** Promotions are now sold by budget: `content_promotion_pricing` (versioned, admin-editable, audited) prices a budget into an impression goal at a cost per 1,000 sponsored impressions; `@abonten/core/content/promotionEstimate` turns it plus `content_audience_snapshot` (nightly measured audience, `content-audience-refresh`) into an **estimated** reach range and refuses budgets the audience cannot take; the campaign snapshots its price. Spend is recognised per valid, de-duplicated delivered sponsored impression (never beyond paid; all of it once the goal is delivered); campaigns stop at goal delivered or run end (`end_reason`), and `content_campaign_metrics` keeps reach, impressions, meaningful views, completions, taps, follows and conversions apart. Sponsored delivery has its own switch (`sponsored_delivery_enabled`) and kill switch (`SPOTLIGHT_PROMOTIONS_KILL_SWITCH`), pacing, least-delivered-first rotation, no repeats within a session, never the advertiser's own post or a restricted publisher; For you / Trending send a rough location only when already allowed. Migrations `20260916130000_content_promotion_reach_model`, `…131000_cloudinary_cleanup_queue_lockdown`, `…132000_content_event_state_in_documents`. The audit also fixed: any signed-in user could insert into (and so have the drain destroy) `draft_asset_cleanup_queue`, which is now server-only and drained by the storage-purge route with a daily sweep for never-registered uploads; video length was never read from Cloudinary (limits unenforced); refused uploads stayed on Cloudinary; content upload signatures went to anyone signed in; cancelling an unpaid promotion left its checkout payable; comments ignored the programme and blocks; signed-out shares were unlimited per address; the CTA ignored sold-out and hidden events; and on mobile the status bar, poster letterbox, per-card follow requests, upload progress over 100 %, a zero-length trim bar, a missing pay-later path and plurals; on web the keyboard paging scrolled the window. Verified with real Cloudinary uploads, Paystack test-mode payments, webhooks and a refund, the Android emulator, and browser runs of web and admin (details in `docs/architecture/spotlight-and-stories.md` §13).

**§34.6 Pre-merge follow-up (2026-09-17).** Spotlight / Story uploads are filed under `content_media/<environment>/<user id>` (`VERCEL_ENV`: production, preview or development), and the daily sweep for never-registered uploads lists only its own environment's folder — the Cloudinary account is shared, so the sweep could otherwise have destroyed another environment's uploads. `run_storage_purge_dispatch` calls the maintenance route for the sweep at most once every 20 hours (it had been calling every 10 minutes until the new route recorded a sweep; migration `20260917090000`). The reach estimate uses one audience share per selectable radius (`content_promotion_pricing.location_audience_share_by_radius`, replacing `location_audience_share_bps`; migration `20260917090100`). The promotions lists show people reached; the mobile feed's tab row fades at its edges and keeps the chosen tab in view; `abonten://spotlight/campaign|post|promote/<id>` and `…/manage` deep links open their screens; the white status bar over media screens uses the native-stack option on Android and `expo-status-bar` on iOS (`MediaStatusBar`). `scripts/perf/promotion-delivery-simulation.sql` simulates a week of delivery; results in `docs/architecture/perf/promotion-delivery-2026-09.md`.

**§34.7 Mobile interaction pass (2026-09-17, branch `feat/mobile-spotlight-stories-ux`).** Spotlight is a **bottom tab** (`app/(app)/(tabs)/spotlight.tsx`, dark tab bar) in the slot Tickets had; the tab is hidden while the programme is off for the person. Tickets is a pushed screen `app/(app)/tickets.tsx` (Account › My Tickets, the menu, payment success via `dismissAll` + push, "Event cancelled" notices); the menu no longer lists Spotlight. The feed shows a page's video only after the shared player has loaded that page's source (`loadedPostId`) and keeps the poster on top until the first frame — the old code mounted the view on the previous source, which flashed the last Spotlight on every swipe. Spotlight card: avatar and name open the profile, inline Follow beside the name (optimistic, rolled back on failure), one action rail (like, comment, share, save with count, Insights on your own post, More), full-width event/place CTA (`contentDestinationCta` — no "View profile" button), redesigned More sheet (tiles, creator tools, Report/Delete apart). Comments open **in place**: the video shrinks to the top (a transform; TextureView on Android so the video follows it) and a panel rises under it; the composer rides the keyboard (`useKeyboardLift`, Reanimated keyboard). Hardware volume: local native module `apps/mobile/modules/volume-observer` (Android `VOLUME_CHANGED_ACTION` and media volume keys while observed; iOS KVO on output volume) — volume up unmutes, volume to zero mutes; needs a native build. Stories: the viewer is a screen (`story/play?queue=…&start=`, and `story/[id]`), not a Modal; a reply bar and reaction row send **private messages** via `POST /api/mobile/content/stories/reply` → `sendStoryReplyCore` → `send_story_reply` (migration `20260917120000`, applied to production): place Stories reuse the place conversation, organizer Stories use one `direct` conversation per pair (the first writer of that type), the message carries `system_data.story_reply` (post, kind, thumbnail snapshot, expiry) shown above the bubble by `StoryReplyContext`; a reaction is also recorded as the Story reaction. Story comments are no longer offered on mobile. Rings are 62 px and the Stories row leads the inbox under the search field. Profile: the first tab is an Events ⌄ / Places selector, then **Spotlights** (Published / Saved / Drafts on your own profile, the published grid on anyone else's — `@abonten/core/content/profileContent`); the profile-header Spotlight strip is gone. The Insights screen adds watch time, average watch, completion rate, profile visits and a Promote / promotion-status card. Shared: `Sheet` animates its keyboard lift; chat bubbles that arrive while a thread is open rise in. Tests: core `storyReply.test.ts`, `profileContent.test.ts`; integration `story-replies.integration.test.ts`.

**§34.5 Open.** Rollout, prices, cancellation refunds and limits are decisions S1–S4; legal F5 (user content) and G4 (promoted content) before opening to Everyone or switching promotions on. Mobile changes reach devices only with a new EAS build or update.

## 35. Holistic production-readiness audit — hardening pass (2026-09-18)

Report: [docs/audit/04-holistic-audit-2026-09-18.md](docs/audit/04-holistic-audit-2026-09-18.md). Branch `audit/holistic-hardening-2026-09-18`. One migration, applied to production via the Supabase MCP and replayed by the integration suite.

**§35.1 Paystack webhook acknowledgement.** `POST /api/paystack/webhook` answered 200 for every `charge.success` once the signature was valid, on the reasoning that `finalizePaystackPayment`'s lock made retries safe. Retries being safe is not the same as retries happening: a transient failure inside finalize (Paystack's verify endpoint timing out, a database error between the lock and ticket issuance) left the attempt in a retryable state and told Paystack the delivery was done, so nothing ever retried it — the customer had paid, no ticket existed, and the only recovery was the person tapping "Check again". `@abonten/services/payments/webhookAck.ts` now decides the status from the finalize outcome: **200** for `succeeded` / `failed` / `not_found` (settled), **503** for `pending` / `fulfillment_failed` / `already_processing` (Paystack redelivers with back-off; every redelivery is one idempotent finalize call). The refund and transfer branches answer 503 on a database error for the same reason; a reference no attempt owns still gets 200. Unit-tested.

**§35.2 One keyboard architecture on mobile.** React Native's `KeyboardAvoidingView` is no longer used anywhere in `apps/mobile`. `@abonten/ui-native` owns the keyboard signal: `useKeyboardLift()` (Reanimated `useAnimatedKeyboard`, moved here from `apps/mobile/src/lib`; no `is*TranslucentAndroid` flags — they are ignored under react-native-edge-to-edge and logged a warning) and `<KeyboardInsetView>` (an `Animated.View` whose `paddingBottom` is the keyboard's overlap frame by frame on the UI thread, minus the view's own measured distance from the window's bottom edge — the same accounting `KeyboardAvoidingView` did, without its one-beat-late `keyboardDidShow` jump on Android). Consumers: the chat thread (`messages/[conversationId].tsx` — no more `useKeyboardVisible` → `scrollToBottom`; an inverted list at offset 0 stays on the newest message when its viewport shrinks), the Android branch of `KeyboardAwareScrollView`, `BottomBar` (its safe-area inset now collapses continuously from the same value instead of snapping when the keyboard lands), `Sheet`, the Spotlight comments panel and the Story reply bar. `EmojiPickerSheet` is now the app's `<Sheet>` (was an RN `Modal` + `KeyboardAvoidingView` + a 120 ms focus timer). Emulator-verified: sign-in form, chat composer (single and three-line draft, send), emoji picker (lift, rejection state, back-twice dismissal) and the Edit-message sheet (footer + input, previously unverified). iOS unverified.

**§35.3 Database.** `20260918100000_audit_fk_indexes_and_constraint_dedupe.sql`: 14 covering indexes for foreign keys the app actually looks up by (content engagement `user_id`s, `content_not_interested.post_id`, `content_campaign.checkout_id`, `content_post.cover_media_id`, `event_reminder_sent.event_id`, `verification_case.place_id` / `organizer_user_id`, the three `credit_reservation` journal columns, `story.user_id`, `wallet.user_id`). The advisor's unindexed-FK list went from 36 to 17; the remainder are `*_by` admin audit columns and were left alone on purpose. The duplicated CHECK constraints on `ticket_type` and `promo_code` (two identical rules under different names) were reduced to one each.

**§35.4 Documentation drift corrected.** §7.2, §7.6 #4–#5, §16 13b and §17 said `ticket.ticket_code` had no uniqueness guarantee and `ticket_type` / `promo_code` no range checks; all three have existed since `20260904131325_phase2_ticketing_constraints.sql` (confirmed live). `docs/mobile/guide/ios-vs-android.md` keyboard row rewritten.

**§35.5 Verification.** `npm run typecheck` (11/11), services unit (118 → 121 with the new `webhookAck` test), core unit (463), the full integration suite on the replayed local stack (56 files), scoped Biome, `check:api-parity` (216 routes), `check:docs`, `npm run build` (see the report for the exact result), Android emulator session on the production API as the founder's account (test message sent and deleted afterwards). **Not verified**: iOS, a live Paystack redelivery against the 503 (the mapping is unit-tested; Paystack's retry schedule is theirs).

**§35.6 Deferred, with reasons.** `multiple_permissive_policies` on `content_comment` / `content_media` / `content_post` SELECT (a policy merge — needs a deliberate decision, no measurable cost at current volume); Postgres minor upgrade and leaked-password protection (Supabase dashboard, founder); the nine Expo patch-version bumps `expo-doctor` reports (`npx expo install --fix`) belong with the next native build, not with a dev client that predates them; the `disableHierarchicalLookup` and `app.json`-alongside-`app.config.js` warnings from `expo-doctor` are documented, intentional choices (`metro.config.js`, `app.config.js`); realtime `postgres_changes` per open thread is correct today and becomes the first thing to replace (broadcast from a trigger) once concurrent open threads number in the thousands.

**§35.7 Verification pass (2026-09-18, same day).** A regression sweep over the audit's changes. Found and fixed one regression the audit itself introduced: the message action overlay (`ContextualActionOverlay`) was still an RN `Modal`; it dismisses the keyboard as it opens, and a Modal window appearing while the keyboard is still sliding away takes over the platform's inset animation, so `useKeyboardLift`'s value on the chat screen froze part-way (composer left 122 dp above an empty bottom edge, reproduced on a clean bundle). The overlay now renders through `@gorhom/portal` with a `BackHandler`, and the rule "no RN Modal on screen while the keyboard moves" is written into `useKeyboardLift.ts`. Also: the `<Sheet>` footer inset moved from a JS `keyboardDidShow` flip to the UI-thread value (BottomBar's formula); the unused `useKeyboardVisible` export was removed; a new integration suite (`webhook-redelivery.integration.test.ts`, 4 tests) proves that concurrent duplicate deliveries, a fulfilment failure followed by a redelivery, a verify outage followed by a redelivery, and a direct second `issue_tickets_for_checkout` call all end with exactly one ticket set, one transaction, one organizer earning and one fee entry. Static sweep: `KeyboardAvoidingView` has no usages; the only `Keyboard.addListener` calls are `useKeyboardHeight` (JS decisions: Back handling, reveal maths) and StoryViewer's "keyboard hidden → stop replying"; every layout-moving value reads `useKeyboardLift`. Database: the 14 audit indexes exist in production, none duplicates an existing index (checked by column set), migration version `20260918093600`. Emulator (clean bundle): overlay → close, normal open/close and four rapid focus/dismiss cycles each end pixel-identical to the untouched thread; scrolling with the keyboard up, leaving with Back while it is up and returning, sending a one- and three-line message all correct. Live Paystack redelivery remains externally unverified.

**§35.8 Deferred items completed (2026-09-18, same day).**
- **Messaging realtime moved off `postgres_changes`** (`20260918120100_messaging_realtime_broadcast_from_database.sql`, applied to production). Triggers on `message`, `message_reaction`, `conversation` and `conversation_participant` call `realtime.send()`; the events and topics are listed in the Realtime note above and typed in `@abonten/core/messagingRealtime` (`MESSAGING_BROADCAST_EVENTS`, `MESSAGING_INBOX_EVENTS`). Why: `postgres_changes` evaluates table RLS per subscriber per change on the Realtime server; a broadcast is authorised once per join. Both apps now hold one private channel per open thread and one `inbox:<me>` channel; mobile mounts the inbox subscription at the app stack host (it used to live in the tabs layout, which a thread opened from a notification may never mount). `realtimeTopic.ts` (the unique-suffix workaround) is deleted. Integration tests prove delivery on the local Realtime server: ids only on the thread, a last-message bump on the inbox, the reaction shape, and an inbox readable only by its owner. **Finding while testing**: the header-only test client authorises only its first private channel (supabase-js refreshes the socket token from the session before each join and falls back to `anon`); the apps hold real sessions, so the suite now uses `createSessionClient` from `setupClient.ts`. **Remaining step**: once the web deploy is live and the mobile update has shipped, remove the four tables from the `supabase_realtime` publication and return them to the default replica identity (a follow-up migration; until then clients on the old build keep working).
- **`get_transaction_refundable_amount` is service-role only** (`20260918120000`): EXECUTE had been re-granted to `authenticated` after the 2026-09-04 revoke, and PUBLIC's default grant was never removed. `issueRefundCore` now calls it with the service-role client after it has authorised the refund. Regression test in `money-path-lockdown`.
- **One SELECT policy per content table** (`20260918120000`): `content_post_select`, `content_media_select`, `content_comment_select` replace the public + author pairs with the identical OR; the performance advisor no longer reports multiple permissive policies. The drafts test now also covers signed-out readers and media.
- **Anchored menus measure after the keyboard has gone** (`useAnchorMeasure`): when the keyboard is up, the long-press dismisses it, waits for the platform's "keyboard hidden" event and one frame, and then measures, so the lifted copy lands exactly over the item's resting position. The header's anchored menu, still an RN `Modal`, therefore also never opens while the keyboard moves.
- **Postgres upgrade unblocked, not run**: the only path the Management API offers is 15 → 17, and it was refused because of the `pgjwt` extension. Nothing used it (no dependent objects, no callers in any application schema); `20260918120200_drop_unused_pgjwt.sql` removes it and the project is now eligible (target `supabase-postgres-17.6.1.166`, GA, estimated up to one hour). The upgrade itself is a major-version change with downtime and no downgrade: the founder schedules it.
- **Leaked-password protection**: requires the Pro plan (the organization is on Free), and it protects nothing here — end users never choose passwords (Google, phone and email one-time codes). Recorded as not applicable.
- **Expo SDK 57 patch versions** applied (`expo install --fix`: expo, expo-image-manipulator, expo-image-picker, expo-location, expo-notifications, expo-router, expo-sharing, expo-updates, expo-constants via the lockfile). `expo-doctor` 19/21; the two remaining checks are the documented intentional ones.

**§35.9 Follow-up fixes (2026-09-18).** (1) Inbox ordering: a last-message update that moves a conversation's time backwards (its latest message deleted) no longer re-seats the row at the top; `bumpConversationInPages` places it by the server's order. (2) Unread counting: `20260918140000` adds `advanced` to the inbox `conversation_update`; the shared `inboxUpdateEffect` (`@abonten/core/messagingRealtime`) raises the badge only for an advanced update from someone else in a thread that is not open, and refetches the inbox for a rollback. Found on device: the badge climbed with each rollback. (3) `getRelativeTime` clamps a future time (device clock behind the server) to now, so a just-arrived message no longer reads "in less than a minute". (4) Realtime rejoin: realtime-js rejoins after a transport failure (measured on device: CHANNEL_ERROR then SUBSCRIBED after a 45-second network drop) but not after a refused join; both apps reopen a channel that is not joined on foreground / tab visible and on TOKEN_REFRESHED (`channelNeedsRejoin`). Verified on the Android emulator against production: a new reply raises the badge by one, a rollback returns the row to its place and the badge to zero, time labels read "less than a minute ago". **Not reproduced on demand**: the stale-token refusal itself (it needs a token to expire while the app is backgrounded); the guard is unit-tested and its trigger paths were exercised.


## 36. Mobile overhaul — offline cache, Spotlight playback, live comments, search relevance and filters, follower counts (2026-09-19)

Design: [docs/architecture/mobile-offline-media-and-sync.md](docs/architecture/mobile-offline-media-and-sync.md). Branch `feat/mobile-spotlight-offline-search-overhaul` (not merged). Four migrations, applied to production via the Supabase MCP and replayed by the integration suite.

**§36.1 Offline-first query cache.** React Query is persisted per account to one JSON file in the OS cache directory (`queryPersistence.tsx`, `@tanstack/react-query-persist-client` — the only new dependency, JS-only). An allowlist with per-kind caps (`queryPersistPolicy.ts`, selection logic in `@abonten/core/query/persistPolicy`) keeps profiles, event/place details, Explore, the Spotlight feed's first page, feature switches, notifications and active promotions; tickets, payments, wallet, finance, verification, messages, comments and search results are never written. Buster = cache version + native version + OTA id; 3 MB / 7-day ceilings. Offline with an expired access token, `SessionProvider` keeps the person signed in on the stored session (supabase-js answers "no session" when the refresh cannot reach the server) and invalidates everything on the first `TOKEN_REFRESHED`; cache files are deleted only on `SIGNED_OUT` or a different account. Programme / own-profile / rewards hooks throw on 401/429/5xx so a transient failure keeps the last good answer. Detail screens render cached data when a refresh fails. The offline pill now actually shows (it sat at opacity 0).

**§36.2 Spotlight playback.** Each feed page owns its `expo-video` player (active / preload / idle — at most three decoders); the single shared, re-sourced player it replaces played the previous clip's audio over the next and left frozen frames. One audio owner (`playbackOwner`), an app-wide persisted mute preference (default muted), ordered teardown (pause → unload → unmount; unmounting a TextureView mid-decode crashed natively), buffer caps (active 12 s / 12 MB, neighbours 3 s / 3 MB; the defaults exhausted the Java heap after ~30 swipes), disk caching, poster until first frame, delayed spinner, retry / "You're offline" state with automatic retry on reconnect. The feed never refetches in the background; a tab re-press scrolls to top then refreshes, pull-to-refresh loads a fresh first page and joins one in flight. Upload screen: explained preview / checking / publishing / error states instead of a black view.

**§36.3 Comments and likes.** Comment lists and likes live in the React Query cache and change only through pure transforms (`@abonten/core/content/commentCache`, `latestIntentToggle`, `postCache`): optimistic "sending" rows reconciled by client id (Retry / Remove on failure), rapid taps coalesced into one final request, post engagement written into every cached copy. `20260919090000_content_comment_realtime_broadcast.sql`: triggers broadcast `comment_insert` (ids only), `comment_update` and `post_counts` to the private topic `content_post:<id>` (join policy `content_realtime_post_read` via `content_realtime_can_join`); an open comment sheet folds new comments in without touching loaded pages. Sheet: 44 pt like column, gutters, the drag drives the video transition on the UI thread.

**§36.4 Search.** `20260919091000_search_relevance_concepts_and_dates.sql`: a `search_concept` vocabulary table (data, service-role only, 51 seeded Ghana-focused concepts) widens each query word to its alternatives while still requiring every word; month names and today / tonight / tomorrow / weekend become an Africa/Accra date window; multi-word queries fall back to any-word / run-together ("afro wave" → "afrowave"); tiers are scored so a literal match always wins; Spotlight search uses the same vocabulary. Mobile search regained a filter button with its own model (`@abonten/core/search/searchFilters`, `SearchFilterSheet`): when, distance, price, event / place category, open now, rating — per-tab applicability, chips, params mirrored into the route, and a Spotlights tab.

**§36.5 Follower counts and profile.** `20260919092000_follow_counts_and_public_profile.sql`: `follow_count` maintained by a trigger on `follow` (backfilled, public read), `viewer_follows` (definer, answers only for `auth.uid()`), and `get_public_profile(username)` — profile, rating, verification, follower count and follow state in one SECURITY INVOKER call (three sequential reads before). `20260919093000` switches `follow_counts` to SECURITY INVOKER (it now reads only the public table). Followers show on every profile including your own; a follow moves the count optimistically.

**§36.6 Settings › Overview promotions and Account entries.** `@abonten/services/promotions/activePromotionsCore` (service role, owner-filtered) lists featured events / places and promoted Spotlights for the web Settings card (the action is now a thin wrapper) and `GET /api/mobile/account/promotions`; labels from `@abonten/core/promotionSummary`. Account gains "Your Spotlights & Stories" and "Saved Spotlights".

**§36.7 Also fixed.** Profile tab crash (`rotate` built from a `withTiming` object). The web build failed to prerender `/weekly` whenever the fallback listed events: `useRequireAuth` called `useSearchParams()` during render, forcing a client-side bailout for every static page holding `EventCard`; it now reads the query string at click time (pre-existing on `main`).

**§36.8 Verification.** Typecheck, core and services unit suites, the full integration suite on the replayed local stack (new: search relevance, comment realtime delivery and join policy, follower counts under concurrency, active promotions), scoped Biome, `check:api-parity`, `check:docs`, web production build. Android emulator against the local stack: one audio track across swipes / tab switches / background, mute persisting across a cold start, heap flat over repeated fast-swipe rounds, live comments and likes between two accounts, rapid-like coalescing, offline cold start showing cached Home / event / profile / Spotlight (a watched clip plays from the disk cache), search filters and relevance queries, overview promotions, follower count. **Not verified**: iOS; mobile needs an EAS build / OTA to reach devices. (Release-build performance, a real Cloudinary upload and the token-refresh window were closed in §36.9.)

**§36.9 Follow-ups closed (2026-09-19, same day).**
- **Signed-in requests never go out anonymous.** With an expired token and an unreachable auth server, `api.ts` sent no token, so routes that also serve signed-out callers answered with the signed-out answer, and it was cached for the signed-in person (Spotlight vanished from the tab bar until stale). The client now sends the stored token (`src/auth/storedSession.ts`). Programme hooks treat a 401 as "keep the last answer". `SessionProvider` refetches everything on the first working token after that happened. Reproduced and verified on the emulator with the auth port blocked and the clock moved 2 h (old client: `/profile` 401, tab gone; new: 200, tab kept, full refetch on reconnect).
- **Search vocabulary tuning loop** (`20260919100000`, applied to production): Admin › Discovery › Search vocabulary shows the searches that found nothing or were never opened, with whether the vocabulary knows their words. It previews what a term would match, and adds, edits, switches off or removes terms (`discovery.configure` + step-up, reason, optimistic concurrency, audit `discovery.vocabulary.*`). It also adds 34 starter terms for the app's own event and place categories (85 in total). Production has too few searches to tune from yet (6 logged, all staff); this is the tool for when it does. Integration suite `search-vocabulary-admin` (4 tests).
- **Release build measured** (emulator, local stack): cold start 1.6–1.8 s. Memory levels off (about 495 MB total, Java heap 38–58 MB) over 160 swipes. Home frame times sit at the emulator's own baseline. Spotlight is heavier, and a Perfetto trace puts that on render-thread TextureView layer work, not app code (numbers in the architecture doc §2). A deferred-neighbour-player change was measured, made no difference, and was not kept.
- **Real Cloudinary uploads** through the new upload screens on the release build: a 0.8 MB clip (no optimised copy needed), and a 9 MB clip that failed once on a network error, recovered with Try again, got its optimised copy (0.93 MB), and played. Both assets deleted.

## 37. Enterprise hardening pass — holistic audit (2026-09-19)

Report: [docs/audit/05-holistic-audit-2026-09-19.md](docs/audit/05-holistic-audit-2026-09-19.md). Branch `audit/enterprise-hardening-2026-09-19`. Two migrations, applied to production via the Supabase MCP and replayed by the integration suite. No table, column, policy or trigger changed.

**§37.1 Every outbound call has a deadline.** `@abonten/core/http/fetchWithTimeout` (`fetchWithTimeout`, `withDeadline`, `HTTP_TIMEOUTS`, `FetchTimeoutError`) is used by every Paystack call (`paystackService.ts`, `paystackTransfer.ts`), Hubtel OTP, Expo push, Google Geocoding (route + server util) and the Cloudinary playback probe. `apps/web/src/lib/email/sendEmail.ts` is the one Resend sender (single instance, 15 s deadline, `emailIsConfigured()`); `packages/services/src/media/cloudinaryClient.ts` is the one configured Cloudinary SDK instance (`destroyAsset()` applies the SDK timeout its typings omit) — the twelve `cloudinary.config()` copies are gone. A Paystack verify that times out still maps to `pending` (retryable), as before.

**§37.2 Typed read path.** `@abonten/services/supabase/publicClient` is created with the `Database` generic. The comment "no generated Supabase types exist in this repo" was stale in ~30 files since `packages/types/src/database.types.ts` landed; every `any` it justified is gone. `@abonten/core/eventAddress` (`readEventAddress`, `readOccurrences`, `normalizeEventRow`) proves the JSON `address`/`occurrences` and geography `location` shapes once at the RPC boundary. `UserPostType.starts_at`/`ends_at` are `string | Date | null`, `capacity` nullable, `Occurrence.id` optional (the synthesised single-date pseudo-occurrence). New row types: `@abonten/types/reviewType` (six list shapes), `eventAnalytics` (the organizer RPC rows), `managedEventType`; `AttendanceRow` and the `EventInsights` result types are real types. RPC calls pass `undefined` only where the SQL parameter is `DEFAULT NULL`; `get_filtered_events` (nullable parameters without defaults) sends nulls through one documented boundary. Nullable columns are honoured (username, tier `type`/`price`/`currency`, event `starts_at`); a registration with no ticket cannot be "checked in"; an unknown payout account type is refused before Paystack; a repeated `?category=` is no longer sent as an array. **Rule**: never re-introduce an untyped Supabase client; when the compiler disagrees with a domain type, the schema is right — fix the type or normalise at the boundary.

**§37.3 Content-Security-Policy and route protection.** `@abonten/core/security/contentSecurityPolicy` (`buildWebCsp`, `buildAdminCsp`, `sentryCspEndpoints`; unit-tested) — set per request in `apps/web/src/proxy.ts` and `apps/admin/src/proxy.ts`. Web: scripts from self + `js.paystack.co` + Google Maps; frames Paystack only; `connect-src` Supabase (https + wss), `api.cloudinary.com`, Maps, `ipapi.co`, Paystack, the Sentry ingest origin; `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`, `upgrade-insecure-requests`, `report-uri` to Sentry's security endpoint (derived from the DSN). Dev adds `'unsafe-eval'` and the HMR socket; Vercel previews add `vercel.live`. `'unsafe-inline'` stays for scripts (Next.js's bootstrap is inline; a nonce policy would make every prerendered page dynamic). Admin allows no third-party script or frame. The session proxy (`src/config/supabase/middleware.ts`) now protects an explicit `PROTECTED_PREFIXES` list (admin, checkout, consent, field, finances, for-you, manage, messages, notifications, plans, rewards, settings, transactions, user-account, wallet); everything else is served, so an unknown URL is the 404 page and `/api/geocode` works before sign-in. **Rule**: a new private section is added to `PROTECTED_PREFIXES` and gets a `robots: noindex` layout; its pages still re-check `auth.getUser()`.

**§37.4 Web pages.** `(pages)/error.tsx` (segment retry, chrome kept), `app/not-found.tsx` + `(pages)/not-found.tsx` sharing `PageNotFound`; a missing event or place is decided in the segment layout (`events/[eventCode]/layout.tsx`, `places/[slug]/layout.tsx`, one indexed lookup): the route has a `loading.tsx`, so a `notFound()` from the page body — or from `generateMetadata`, which Next streams for JavaScript-capable crawlers too — only arrives after a 200 shell; a layout renders before the shell is flushed, so the response is a real 404; root `title.template` "%s | Abonten Hub" and thirteen hand-written suffixes removed; noindex layout metadata for Settings, Finances, Transactions, Manage, Checkout, Auth, Admin, Consent, Field Ops; `generateMetadata` + canonical for `/explore/[location]`, `/events/location/[location]` (+ `explore/[type]`), public profiles; one `<main>` (the root layout and three group layouts no longer nest their own); `Header` is a `<header>`; the stub `/around-you` page and the dead `/api/user-profile` route removed.

**§37.5 Accessibility tokens.** New `--primary-text` token (light: `171 70% 27%`, the mint darkened to at least 4.5:1 on white and background; dark: the mint) in web `globals.css`, mobile `global.css` and `@abonten/ui-tokens/palette`; `theme.extend.textColor.primary` in both Tailwind configs resolves `text-primary` to it while `bg-`/`border-`/`ring-primary` keep the mint (`text-primary-fill` opts back in; `text-primary-foreground` unchanged). `--muted-foreground` light 46 % → 42 % (4.35:1 → AA on `--muted`). `MaskIcon` with an empty `alt` is `aria-hidden` (was an unnamed `role="img"`). Mobile: `PressableScale` defaults `accessibilityRole="button"`; 123 raw `<Pressable>`s that handle a press carry the role (codemod; backdrops/gesture hosts untouched). The axe scan of the public web surface passes at the serious/critical level.

**§37.6 Configuration at boot, structured logs, admin modules.** `@abonten/core/env/checkEnv` (`checkEnv`, `enforceEnv`) + `instrumentation.ts` in web and admin: required variables verified once per process, throw when `VERCEL_ENV=production`, log names otherwise (values never printed); admin requires a non-empty `ADMIN_EMAIL_ALLOWLIST`. `@abonten/core/logger` emits one JSON line per entry on a production server (`formatStructured`, unit-tested); consoles elsewhere unchanged. `apps/admin/src/server/actions.ts` (2,157 lines, 87 actions) is now sixteen domain modules under `src/server/actions/` + `_shared.ts`.

**§37.7 Browser suite.** `apps/web/e2e/` (Playwright + `@axe-core/playwright`; `npx playwright test` in `apps/web` after a build; CI job `build-and-e2e-web` replaces `build-web`): headers (CSP, XFO, HSTS), `robots.txt`/`sitemap.xml`, sign-in redirect with `next`, noindex on private vs public pages, mobile API 401, webhook 401, unknown-URL and missing-event 404s, canonical URL and title on a location page, Open Graph + JSON-LD on a live event and place page, axe on `/`, `/explore/accra`, `/help`, `/legal/terms`, `/weekly` and a live event/place. Its first run found §37.3's redirect-to-sign-in, §37.4's soft 404 and §37.5's contrast failures.

**§37.8 Database.** `20260919110000_audit_grants_and_realtime_publication`: five trigger functions no longer executable by client roles; `message`, `conversation`, `conversation_participant`, `message_reaction` out of `supabase_realtime`, replica identity default (the trigger-broadcast cut-over of §35 is complete — no store build of the mobile app existed, the web build with broadcasts was live). `20260919120000_restore_checkout_sweep_grants`: the same audit migration had revoked the three `expire_stale_*_checkouts` sweeps from `authenticated`; the integration suite showed fourteen call sites run them on demand with the caller's session as a self-heal, so the grant was restored (SECURITY DEFINER, idempotent, only rows past expiry). The revoke was live for about fifty minutes with the cron sweep running throughout.

**§37.9 Repository.** Root `.gitattributes` pins LF (Biome writes LF; on Windows a formatting pass showed 138 spurious CRLF modifications). `twilio` removed (never imported); `@playwright/test` and `@axe-core/playwright` added as web dev dependencies.

**§37.10 Verification.** See the report §5 for the commands and counts; every suite named there was run on the final tree. Not verified: iOS; a CSP violation report arriving in Sentry; mobile screen readers on a device.

**§37.11 Adversarial pre-merge pass (same day).** Report §8. Six more defects: three introduced by this pass, one pre-existing bug its own typing exposed, one measured scalability defect, one wrong claim in the report.

- **`/api/geocode` was left open.** Inverting route protection (§37.3) removed the incidental guard on a proxy to a **billed** Google API. A route-by-route diff of old versus new protection across every page and API route found it to be the only route whose protection changed. The route now authenticates itself (`auth.getUser()` → 401 JSON, never a 307 to HTML — an API route owes a caller a status, not a sign-in page) and rate-limits per account. Its only caller is the Field Ops lead territory form, which is behind `/field`; the "used before login" comment was wrong. **Rule**: `PROTECTED_PREFIXES` guards page sections; an API route under the proxy matcher must check `auth.getUser()` itself.
- **A deadline that became a page crash.** `geocodeAddress` threw `FetchTimeoutError` into the render of `/explore/[location]`, `/events/location/[location]`, `explore/[type]` and the event detail page, none of which catch — a slow Google would have taken those pages to the error boundary where previously they waited. It now returns `{ lat: null, lng: null, error }`, the shape it already used for an unresolvable address.
- **Null coordinates reached the discovery RPCs** (pre-existing; the honest return type surfaced it). `get_nearby_events` / `get_events_in_window` / `get_similar_events` take `double precision`, so a failed geocode queried with nulls and rendered as an empty city. The two location pages now render `LocationUnavailable` (with the location picker still on screen); similar-events and the event page skip the nearby query.
- **Admin › Monitoring scanned the largest table.** `order by checked_at desc limit 300` then de-duplicating per check in JavaScript could not use `(check_key, checked_at DESC)`: `EXPLAIN (ANALYZE, BUFFERS)` on production gave a parallel sequential scan plus a top-N heapsort, **895 ms / 2,207 buffers** over 127,462 rows (29 MB, 30-day retention, ~9,360 rows a day). Replaced by one indexed lookup per key from the exported `HEALTH_CHECK_KEYS`, issued together: **0.114 ms / 4 buffers** each. Same semantics, deterministic order.
- **One dependency advisory is runtime, not build tooling.** `expo-router@57 → query-string@7 → decode-uri-component@0.2.2` is reachable from incoming deep links. Moderate denial of service via malformed percent-encoding: a crafted link freezes the app, no data exposure. *(Superseded by §38, which fixed it. Two details here were wrong: the call site is `build/react-navigation/core/getStateFromPath.js`, not `build/fork/getStateFromPath.js`, and "not fixable here" was wrong — it is fixable without an npm override. The other two advisories named here, `metro` → `image-size` and `@expo/config-plugins` → `xcode` → `uuid`, are genuinely build-time; `image-size` has since cleared.)*

**New guards, each proved to fail before it was trusted.**
- `packages/services/src/__integration__/function-grants.integration.test.ts`: thirteen session-callable RPCs must stay callable, seven service-role-only ones must stay refused. Re-running the incident on the local stack (revoke → test fails with the exact diagnosis → grant → passes) proved it. The refusal half calls each function as the service role first, so `PGRST202` can only mean "invisible to this role", never "wrong arguments".
- `apps/web/e2e/route-protection.spec.ts`: all fifteen private prefixes redirect to sign-in, ten public paths do not, and `/api/geocode` answers 401 JSON.
- `apps/web/e2e/csp-reporting.spec.ts`: intercepts the configured `report-uri`, injects a script from a disallowed origin, and asserts the browser posts a real `csp-report` naming `script-src` and the blocked origin — verified against a server started with a DSN. It also asserts the policy refuses nothing of the app's own. Skips, with the reason, where no DSN is configured.
- `scripts/check-mobile-a11y.mjs` (`npm run check:mobile-a11y`, in CI): fails if any `<Pressable>` with an `onPress` carries no accessibility role, keeping §37.5's codemod from rotting. Proved by planting a violating component.

**Grant incident, root cause.** The migration read "pg_cron job" as "scheduler only"; the application also runs those sweeps with the caller's session as a self-heal. Nothing in TypeScript, review or the build can see a grant — only a live `42501` can. All 215 RPC names in the codebase were cross-referenced against `has_function_privilege('authenticated', …)` on production: seventeen are not executable by `authenticated`, and every one was traced to a `ServiceRoleClient` call site (two have no call site at all). **No other latent grant bug exists.**

**Verified on the final tree**: typecheck 11/11; core 561, services 121; integration 63 files / 568 tests on the replayed stack (the grant guard is the new file); `check:api-parity` 217; `check:mobile-a11y`; `check:docs`; web and admin production builds; Playwright 44 passed / 1 skipped (the report-uri test, which passes when a DSN is present — proved separately). **Not verified**: iOS; VoiceOver and TalkBack announcement quality; Sentry's own rendering of a CSP report.

## 38. Expo SDK and mobile dependency migration (2026-09-19)

Branch `chore/expo-sdk-57-dependency-migration`. Full report: `docs/audit/06-expo-dependency-migration-2026-09-19.md`. Prompted by the `decode-uri-component` advisory §37.11 recorded as unfixable. Both halves of that framing turned out to be wrong: no SDK upgrade was appropriate, and the advisory was fixable.

**38.1 The target SDK is the one already in use.** `expo@57.0.24` is npm's `latest`; `expo install --check` reports the project up to date; every native module matches Expo's SDK 57 pin. SDK 58 exists only as `58.0.0-preview.3`, is built on **React Native 0.88.0-rc.0**, and has no published changelog. Decisively, **`expo-router@58.0.4` still declares `query-string: ^7.1.3`** — so SDK 58 would put a production app on an RN release candidate *and* still ship the vulnerable package. **Rule**: check `bundledNativeModules.json` and `api.expo.dev/v2/versions/latest` before proposing an SDK move; "newer exists" is not a reason.

**38.2 `@sentry/react-native` stays at 7.11.0, deliberately.** 7.x is unmaintained upstream (last release 7.13.0, 2026-02-12), but Expo pins `~7.11.0` in SDK 56, 57 **and** the 58 preview, and Sentry 8's breaking changes are native (Sentry AGP 6, Sentry CLI 3, iOS 15+). Moving would leave the supported set and fail `expo-doctor` for no security gain — `npm audit` flags Sentry only through the `expo → @expo/config-plugins → xcode → uuid` build-time chain. Move it when Expo moves its pin.

**38.3 `decode-uri-component`, fixed without an override.** Metro resolves the module to `apps/mobile/vendor/decode-uri-component.js`, the same mechanism `metro.config.js` already used to pin `color-string` to v1. The npm graph is untouched, so expo-router's Node and web builds keep the package they declare, and **`npm audit` stays at 18 moderate — which is correct**: the package is still in the tree, just not in the app. Proved against the shipping artifact: `decodeComponents` (the bisecting function) is present in the bundle before and absent after; `decodePass` / `decodeUriComponent` are absent before and present after. The replacement keeps the original's quirks on purpose, including the lossy tokeniser that drops a bare `%`; three attempts were needed before the differential test agreed. Measured cost of the original: **6,115 ms for a 1,352-character link**, versus 8.8 ms.

**38.4 How exposed it really was.** A grep for importers of the module that calls `queryString.parse` finds none, which is misleading — react-navigation's core barrel re-exports it and `useLinking.native` / `useLinkBuilder` name it as their default. But that default is never taken: `ExpoRoot` renders `fork/NavigationContainer` with `linking={store.linking}` → `LinkingContext.options` → and `getLinkingConfig()` always defines `getStateFromPath`, which delegates to `fork/getStateFromPath`, a fork that uses `expo.parseQueryParams` and never imports `query-string`. Sending the payload to a device, with and without the fix, showed no difference. **So this closed a latent risk, not an exploit in flight** — and §37.11's "runs on every deep link" was too strong, with the wrong call site named. **Rule**: liveness is not reachability; check whether a default parameter is ever actually taken.

**38.5 Guard.** `npm run check:deep-link-decoder`, in CI: fails if the Metro alias goes, if the replacement disagrees with the original on any of **3,033** corpus inputs, if it stops being linear, or if **`expo-router`'s `getLinkingConfig` stops defining `getStateFromPath`** — the one fact that keeps the vulnerable path unused. Every failure mode was proved by causing it.

**38.6 Config corrections.** `app.config.js` now takes the config Expo passes in rather than re-reading `app.json` (`expo-doctor` 19/21 → 20/21; the resolved `expo config --type prebuild` is identical, Maps key included). The `disableHierarchicalLookup` comment credited the flag with fixing EAS entry resolution, which is false on SDK 57 — Gradle resolves the entry via `expo/scripts/resolveAppEntry`, which never reads the Metro config. What it actually buys, measured: **851 KB** of duplicate nested modules (10,224,011 vs 11,075,409 bytes). Kept, with the real reason. That is the one remaining `expo-doctor` finding and it is deliberate.

**38.7 Nothing else changed.** `react-native-url-polyfill`, `@gorhom/portal`, `react-dom` and `react-native-web` were each checked for removal and are all genuinely used. No dependency added, removed or pinned; `package-lock.json` untouched.

**38.8 Release candidate.** The EAS cloud build was **blocked**: the account's free-plan Android builds are exhausted until 2026-10-01, and `eas build --local` needs macOS/Linux. Built through Gradle instead (`expo run:android --variant release`) — same clean prebuild, all five config plugins verified applied, same Metro bundle including the alias, R8 on; signed with the debug keystore, so emulator-valid and not Play-valid. Verified on Android 17 / API 37, release build (no `DEBUGGABLE` flag): cold start 3360/2443/2496/2315 ms; Sentry initialises with the right DSN and native integrations; email OTP sign-in end to end (and the OTP rate limiter fires correctly); push permission → a new `ExponentPushToken` row in `device_token` at the moment of granting; deep links `/events/<code>`, `/weekly`, `/spotlight`; a 1,391-character malformed link with no ANR; all five tabs; search type-ahead and full search; **messages — send reached the database and a server-side insert appeared in the open chat with no refresh, confirming the build does not depend on the `supabase_realtime` publication §37 removed the messaging tables from**; tickets; composer stays anchored above the keyboard; **zero FATAL EXCEPTION and zero ANR** across the session.

**38.9 Not verified**: the map (no event or place data in Accra, so it renders its empty state and no `MapView` mounts — the Maps key *is* in the generated manifest and no authorisation error appeared); checkout, payments, event/place creation, media upload, reviews, reports; iOS; and an EAS cloud build, hence upload-keystore signing, Sentry source-map upload and a Play-installable artifact.
...
## 45. Global platform: markets, money, payments, time and locale (2026-09-24)

A country is configuration, not code. Reference: `docs/architecture/global-platform.md`; runbook `docs/admin/markets.md`.

- **Market model** (migration `20260924100000`): `market` (status draft/preparing/ready/live/paused/maintenance, exactly one `is_default` = Ghana), `market_payment_provider` (env variable **names**, settlement currency, accepted currencies, `payouts_enabled`), `market_payment_method`, `market_payout_method`, `market_region`, `market_event` (audit), `market_readiness_run`, `currency`, `feature_flag`, `exchange_rate` / `exchange_rate_config` (+ pg_cron `exchange-rates-refresh` → `/api/jobs/exchange-rates`). All service-role only. `market_transition()` is the only status change; mark_ready/activate/resume need a passing readiness report; the default market cannot be paused. Seeds: GH live; NG, KE, ZA, CI (Paystack), GB, US, FR, DE (Stripe) drafts, everything off.
- **Domain columns** (`…100100`): `event.country_code/timezone/currency` and `place.country_code/timezone` NOT NULL (backfilled from the default market), `ticket_type.currency` equals its event's (trigger), `user_info.country_code/display_currency/locale/distance_unit`; `transaction.paystack_reference` renamed `provider_reference` plus `provider`, `provider_transaction_id`, settlement, provider fee, tax, country; `payment_webhook_event`; payout and payout-account market columns; every `'GHS'` / `Africa/Accra` literal removed from SQL functions (sanity blocks fail the migration if one returns). Credit: one currency per account (`credit_account` unique per user), system ledgers per currency.
- **Time** (`…100200`): reminders, search, digests, weekly and place-visit days in the subject's zone; `phone_otp_state.provider`.
- **Reporting** (`…100300`): `admin_rewards_overview` and `content_admin_overview` take `p_currency` and list the currencies with activity. **Listings** (`…100400`): discovery RPCs return `timezone`, `country_code` and the event's currency. **Home-country guard** (`…100500`): clients cannot write `user_info.country_code`.
- **Code**: `@abonten/core/{money,market,phone,geo,time,units,flags}`; `@abonten/services/payments/providers` (Paystack + Stripe adapters, registry), `chargeInit`, `finalizePayment`, `webhookCore` (`/api/payments/webhook/{provider}/{country}`), `markets/*`, `fx`, `flags`, `geo/locationResolution`, the OTP router (Hubtel / Twilio Verify); Admin › Markets; the market context on web (`useMarketContext`) and app (`MarketProvider`, persisted); Settings › Region & currency.
- **Removed**: `finalizePaystackPayment`, `verifyPaystackPaymentCore`, `submitPaystackChargeOtpCore`, `paystackInit`, `gateway/paystackService`, `gateway/paystackTransfer`, `hubtelOtpClient`, env `PAYSTACK_TRANSFERS_ENABLED`. `/api/mobile/paystack/momo-networks` moved to `/api/mobile/payments/momo-networks`; the old path stays as a re-export for app builds already on phones (exempted in `scripts/check-mobile-api-parity.mjs`) until those builds are retired.
- **Hardening pass** (2026-09-25, migrations `20260925100000`–`…100500`): money columns hold three decimals and every ledger/refund/credit/report function rounds with the row's currency (`currency_minor_units`, `money_round`, `minor_to_major`, `major_to_minor`; a market may only use a currency in the `currency` table); `admin_dashboard_kpis` / `admin_finance_overview` / `admin_platform_analytics` take `p_currency` (console currency switcher); checkout pays with a saved instrument **or** a market-configured method on the provider's page (`payments/paymentChoice.ts`, `GET /api/mobile/payments/options`, web `getCheckoutPaymentOptions`; `checkout.<provider>` flags gate providers); saved cards are bound to the provider account that made them; `payment_orphan_capture` records and auto-refunds money captured after its attempt closed or for the wrong amount; `record_payout_reversal` handles `transfer.reversed`; a refund confirmation that overtakes the refund request is redelivered (503); `market.version` + `market_transition(p_expected_version)` pin activation to the checked configuration; `listing_market_visible()` hides not-live markets' listings from every discovery function and non-live markets refuse checkout and new listings; Weekly weeks, the digest hour, place opening hours and "today/this weekend" search use the local zone; `market.display_config.priceScale` sizes price filters; `market_payment_provider.options` carries per-country provider facts. The unused `markets.browse_abroad` flag was removed. Admin console times are UTC, labelled.
- **Owner decisions still open**: which country opens next and when; counsel and tax sign-off per country; provider accounts (Paystack NG/KE/ZA/CI, Stripe GB/US/EU), Twilio Verify and Open Exchange Rates keys; Paystack transfers still unverified live (F2).

## 46. Full-system adversarial audit (2026-09-25)

Report: `docs/audit/07-full-system-audit-2026-09-25.md` (28 findings, how each was reproduced, rollout order). Branch `audit/full-system-2026-09-25`.

- **Payments**: attempts bound to one charge (`metadata.charge_minor`), claimed before the provider call, never re-referenced; one open attempt per checkout (partial unique indexes); a succeeded attempt whose reference isn't its transaction's is refunded as an orphan capture; basket totals from the server's preparation; `computeCheckoutFee(…, currency)`.
- **Data API guards** (migrations `20260925110000`–`110800`): `guard_listing_market_columns` (event/place: featured, currency, country, zone, publish/archive dates are the service's; no direct client INSERT); `create_event`, `create_place`, `issue_free_ticket` service-role only (services call them with the service client and check `isAccountRestricted`); attendance INSERT revoked; tickets: clients only `active ↔ used`; payout_account client writes revoked; booking transitions; promo codes organizer/staff-only; attachment delete own-only; place self-review refused; ticket price precision; text length limits; analytics throttle; payout currency/precision/restriction checks.
- **Abuse**: restricted accounts refused on every Server Action (middleware checks `next-action` POSTs on public paths too); OTP per-country hourly ceiling + no codes for draft/preparing markets; listing-market and market-context point lookups rate limited.
- **Global**: provider variable-name allowlist (`providerEnvNameProblem`), exchange-rate app id / URL allowlist, `marketForPhone` (shared calling codes), `formatMoney` on every money display, payout accounts filtered by currency in both withdraw screens.
...
### 46.2 Incident recovery and final release gate (2026-09-25)

Report: `docs/audit/09-incident-recovery-and-release-gate-2026-09-25.md`. Branch `gate/final-release-2026-09-25`.

- **Incident 1 — event creation** 06:15:57 → 17:24:52 UTC: `create_event` (INVOKER) began reading service-only `market`/`currency` in `global_markets_domain_columns`; the apps called it as the organizer. Fixed by the gate code (service-role call). No failed request observed; no data or money affected. The 22–25 Sep gap was inactivity (no `create_event` call in the API log).
- **Incident 2 — organizer dashboard** 08:51:08 → 18:27:26 UTC: `default_market_currency()` (INVOKER, reads `market`) inside the sales timeline. Fixed by `20260925111500` (the three `default_market_*` helpers are SECURITY DEFINER), applied live. Rule: a client-callable function never reads a service-only table with the caller's rights (`session-rpc-reachability` test).
- **Fixes**: restricted organizer's edit → 403; Admin event detail shows market + zoned start; push tokens Expo-shaped, ≤ 10 per account (`20260925111600`); search ranking capped at `greatest(p_limit, 1500)` matches per branch (`20260925111700`, 2.6 s → 115 ms, results unchanged below the cap); readiness `utc_calendar` blocks activating a non-UTC+0 market until four UTC calendars are local; replaced/abandoned images destroyed only via `destroyAssetIfUnused` (one organizer could delete another's image); `place_is_open_now` works for signed-out visitors again (`20260925111800` — its dead fallback called `default_market_timezone()`, which visitors may not execute). Deployed 2026-09-25: code 20:15 UTC (`be928ad7`), then 111600–111800 at 20:19–20:22; production smoke 32/32 (report 09 §12).
...
- **Final verification (same day)**: `PAYSTACK_WEBHOOK_SECRET` must equal `PAYSTACK_SECRET_KEY` (Paystack signs webhooks with the secret key) and the registry refuses a different value; a malformed key is refused under `PAYMENTS_MODE`; a live key is refused when `VERCEL_ENV` ≠ `production`; the sweep and health row cover `fulfillment_failed` (`20260925121000`); the admin refund moves money only (tickets stay active) — the buyer's Cancel ticket is the path that cancels and refunds. The new smoke check found on production that Paystack's `verify` answer for a never-opened hosted page (`authorization: {}`) failed the strict parser and left abandoned attempts open; fixed in `paystackApi.ts` (`d07b8a09`) — every payment test simulates Paystack, so shapes like this only show up against the real API. Runbook `docs/finance/paystack-live-cutover.md` 1.1 has the 13-step sequence and the record chain for the GH₵1.05 test.
- **Risks closed (same day, `docs/audit/10…` §10)**: previews have their own Supabase project **Abonten Preview** (`qasxtirvfbreygsqwwat`, eu-west-3, schema replayed with `supabase db push --db-url` — fine for a fresh project; still never against production) and the production URL/anon/service-role variables are Production-only; `@abonten/core/env/productionProject` makes both apps refuse a preview/development deployment on the production project; Admin › Finance → Refund cancels the order's tickets first (`cancelTicketsForTransactionCore`); attempts become `refunded` with their transaction; Paystack's verify parser is tested on the four real shapes (card success, momo success, reversed, failed, plus abandoned).
