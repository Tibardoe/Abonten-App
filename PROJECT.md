...
**Verification:** 42 new unit tests; 4 new integration files (finance parity, dashboard definitions, analytics zero-fill and suppression, RPC authorisation); full suite 50 files / 449 tests on a clean replay; production RPCs read back via MCP; every rebuilt page rendered end to end against a local stack signed in as a throwaway staff account (this caught a server→client function prop before it shipped). Not done: a driven-browser check of `InfoTip` keyboard behaviour (no UI test harness exists; behaviour verified by markup and code review) and the two `.env.local` legacy keys, which stop the admin dev server talking to production locally.

**Deferred:** refunds list still calls `get_transaction_refundable_amount` per row; rewards / discovery / field-ops sub-pages keep their own summaries and silent `.limit()` caps; no region mapping of events; `next-themes` remains unwired; `test:db:up` needed a `SUPABASE_TEST_PORT_OFFSET` because Hyper-V reserves the default ports on this machine.
