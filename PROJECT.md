...
- **One dependency advisory is runtime, not build tooling.** `expo-router@57 → query-string@7 → decode-uri-component@0.2.2` parses incoming deep links (`build/fork/getStateFromPath.js`). Moderate denial of service via malformed percent-encoding: a crafted link freezes the app, no data exposure. Not fixable here — the advisory covers `<=0.4.2` and the first fixed release (`0.5.0`) is ESM-only while `query-string@7` `require`s it, so an `overrides` pin would break the Metro bundle. Clears with a future Expo SDK; check it at the next bump. The other two (`metro` → `image-size`, `@expo/config-plugins` → `xcode` → `uuid`) are genuinely build-time.
