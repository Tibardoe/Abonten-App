...
Verification status (2026-09-12): **SOURCE VERIFIED and locally served** — `npm run check:docs` passes the `security-txt` rule; a signed-out `curl` against `next dev` returned `200 OK`, `Content-Type: text/plain; charset=UTF-8`, the full file body and no redirect, and the rendered `/legal/security` page contains the `id="responsible-disclosure"` anchor the `Policy` field points at. **PRODUCTION VERIFIED (2026-09-12, 19:07 UTC):** after the merge to `main` (commit `9b6c8b23`, Vercel production deployment `dpl_6HMSpPGYBNFkmeubVkrYZuKifNAJ`, state READY), `curl -i https://abontenhub.com/.well-known/security.txt` returned `200 OK`, `Content-Type: text/plain; charset=utf-8`, `X-Matched-Path: /.well-known/security.txt`, `Server: Vercel`, the full 364-byte body, and no redirect; `https://abontenhub.com/legal/security` contains `id="responsible-disclosure"`; `/terms` still redirects `308` to `/legal/terms`. Re-verify after any change to the proxy allowlist or the file.
