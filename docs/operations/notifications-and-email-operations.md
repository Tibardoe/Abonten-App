...
3. **Email:** health `resend`; Resend dashboard → Emails (delivered/bounced/complained) and Suppressions; `no_email` = phone-only account; `opted_out` = reward emails off. Sign-in codes: Supabase Auth logs and the SMTP setting (Resend); the built-in Supabase sender is rate-capped and must not be relied on in production.
