...
| **Refunds** | Transactions in `refund_pending` / `refunded`, charged vs refundable (the refundable amounts for the whole page come from one call to `admin_transaction_refundable_amounts()`, which evaluates the same `get_transaction_refundable_amount()` per row) | none (retry via Transactions › Refund) |
...
- Money sums are rounded to the pesewa in SQL. Summing `numeric` columns as JavaScript floats had produced values like 6.999999999999886.
