...
| `unit-tests` | `npx vitest run` (core), `npm run test -w @abonten/services`, `npm run check:api-parity`, `npm run check:mobile-a11y`, `npm run check:deep-link-decoder` | none | Pure logic; mobile API ↔ client parity; every pressable view announces a role; the bundled deep-link decoder still matches the package it replaces and is still wired in |
