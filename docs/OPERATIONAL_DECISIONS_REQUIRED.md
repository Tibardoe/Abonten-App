...
| S2 | Promotion pricing and reach-estimate assumptions | `content_promotion_pricing` (Admin › Spotlight & Stories › Settings) | Budget-based since 2026-09-16: GH₵ 20–5,000 (steps of GH₵ 5; suggested 20 / 50 / 100); runs of up to 3, 7 or 14 days; GH₵ 11 per 1,000 sponsored impressions; 1.5 impressions per person; ±20 % range; **no planning floor**, so nothing can be sold until real audience data exists or a floor is set; 30 % daily fill; 60 % max reach share; 30 % audience for a location target (any radius); refuse under 50 % deliverable; pacing ×2; cash only. All are uncalibrated assumptions. | Set a conservative planning floor for the staff/beta phase; after the first month compare estimated reach with delivered reach and impressions per cedi with event and place featuring, then adjust the cost per 1,000 and the floors. | Open |
