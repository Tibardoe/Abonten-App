...
| S2 | Responsible-disclosure policy | Public Security page; `incident-response/vulnerability-report.md` | security@abontenhub.com is published (2026-09-12) and the Security page asks researchers not to access others' data, disrupt the service or demand payment. No formal policy, response target or safe-harbour statement exists. | Publish a short policy (scope, what to include, response target, safe-harbour wording drafted by counsel) and a `/.well-known/security.txt` pointing at it. See [specifications/support-and-security-contacts.md](specifications/support-and-security-contacts.md). | Open (contact address done) |
