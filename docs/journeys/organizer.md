...
| **Publish** | Publish now or later; edit any time | `event.status = published`; appears in discovery RPCs and search at once; organizers' followers get a capped alert when Discovery is live | Not appearing (moderation, unpublished) | troubleshooting |
