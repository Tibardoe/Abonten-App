...
- **Web** (browser against the same stack, 2026-09-17 follow-up): a place Story reply and a heart reaction sent from the viewer without closing it ("Reply sent · View chat"), both stored in the place conversation with `story_reply` context; the chat shows "You replied to their story" with the preview. Mobile save count re-checked on the emulator (0 → 1 → 0).
