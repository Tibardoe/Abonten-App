...
| Spotlight, Stories, follows and promoted Spotlights (ships switched off) | W M A | watch: audience of the programme; post: organizers, place owners, staff; promote: publishers | `/spotlight`, `/spotlight/<id>`, `/stories/<id>`, Stories row in Messages, `/manage/spotlight`, Follow on profiles and places; app `spotlight/*`, `story/<id>` | upload to Cloudinary → register → publish (`content_post_publish`) → ranked feed / Story tray; promote → pay (cash) → review → time-billed delivery → refund | `content_program_setting`, `follow`, `content_post`, `content_media`, engagement and telemetry tables, `content_campaign*`; `content_feed`, `content_story_tray`, `content_view_ingest`, `content_campaign_transition` | actions `src/actions/content/*`; API `content/**`, `checkout/spotlight-promotion-attempt` | programme off, not eligible to post, Story ended (410 → publisher), post hidden, promotion rejected (refunded) | architecture/spotlight-and-stories |
...
| Place reviews + replies | W A | Cu / Ow | one per person; owner response; preview + `/places/<slug>/reviews` (breakdown, star filter, sort, Helpful, share, report, block) | `place_review`, `place_review_photo`, `place_review_helpful`; `review_list`, `review_summary`, `review_set_helpful` | actions `postPlaceReview`…, `respondToPlaceReview`, `getReviewPage`, `setReviewHelpful`; `reviews/reviewListQuery` | Content | help; architecture/reviews-and-account-setup |
...
| Event reviews | W A | eligibility (checked-in, ended); preview + `/events/<code>/reviews` (breakdown, star filter, sort, Helpful, share, report, block) | `event_review*`, `event_review_helpful`, `review_drafts`; `review_list`, `review_summary`, `review_set_helpful` | actions `postEventReview`…, `getReviewPage`, `setReviewHelpful`; `reviews/*` | Content | help; architecture/reviews-and-account-setup |
