...
**Mobile** (`apps/mobile`): `app/(app)/weekly/index.tsx` and `app/(app)/weekly/[scope]/[week].tsx` over `src/components/weekly/WeeklyScreen.tsx`; `WeeklyTeaserCard` at the top of Explore; a drawer row. Deep links `abontenhub.com/weekly`, `/weekly/<area>`, `/weekly/<area>/<monday>` (and `abonten://weekly/...`) are mapped in `app/+native-intent.ts`; `/weekly` is in the Android App Links intent filter, which takes effect with the next native build.
