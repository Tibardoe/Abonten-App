...
**Mobile** (`apps/mobile`): `app/(app)/weekly/index.tsx` and `app/(app)/weekly/[scope]/[week].tsx` over `src/components/weekly/WeeklyScreen.tsx`; the edition as the first slide of the Explore Spotlight (`DiscoveryHero` / `spotlight/SpotlightCards.tsx`, since 2026-09-16); a drawer row. Deep links `abontenhub.com/weekly`, `/weekly/<area>`, `/weekly/<area>/<monday>` (and `abonten://weekly/...`) are mapped in `app/+native-intent.ts`; `/weekly` is in the Android App Links intent filter, which takes effect with the next native build.
