...
| F17 | INFO · DEPENDENCIES | Monorepo | `npm audit`: 20 advisories, all transitive. Two are genuinely build-time only (`metro` → `image-size`; `@expo/config-plugins` → `xcode` → `uuid`, used by prebuild). **One is not**: `expo-router` → `query-string@7` → `decode-uri-component@0.2.2` is in the mobile app's runtime URL parsing (`getStateFromPath`), so it runs on every deep link | Upstream pins | Non-breaking `npm audit fix` applied. The runtime one was later fixed without an override — see `06-expo-dependency-migration-2026-09-19.md`. The reasoning recorded here (an `overrides` pin breaks the Metro bundle) is correct; the conclusion drawn from it, that nothing could be done, was not |
...
- **Expo SDK upgrade** for the three remaining `npm audit` advisories, none of which can be resolved from this repository:
  ...
  - `expo-router` → `query-string@7` → `decode-uri-component@0.2.2` **is in the mobile runtime**. **Fixed after this report — see `06-expo-dependency-migration-2026-09-19.md`.** Two claims above were wrong: the call site is `build/react-navigation/core/getStateFromPath.js` reached through react-navigation's core barrel, not `build/fork/getStateFromPath.js` (which does not import `query-string` at all); and "not fixable here" was wrong. An `overrides` pin would indeed break the bundle, but a Metro resolver alias to a vetted drop-in replaces the bundled copy without touching the dependency graph. The severity finding stands and was measured: a 1,352-character link blocks the JS thread for 6.1 seconds.
...
| F22 | LOW · ACCURACY OF THIS REPORT | §6 said every remaining `npm audit` advisory was build-tooling only. One is not | `npm ls decode-uri-component` → `expo-router@57 → query-string@7 → decode-uri-component@0.2.2`, reachable from deep-link parsing at runtime | F17 and §6 corrected with the exploitability. The named call site (`build/fork/getStateFromPath.js`) was wrong and the "cannot be fixed here" conclusion was wrong; both are corrected in `06-expo-dependency-migration-2026-09-19.md`, which fixes the advisory |
