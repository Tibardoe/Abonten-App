...
- **Recommended solution**: **product decision required**, not a mechanical fix — options are (a) stop hard-deleting events with earnings/promotion history at all (soft-delete/archive flag instead, matching the `moderation_state` pattern already used elsewhere), or (b) keep hard-delete but drop the `event_id IS NOT NULL` half of the earning CHECK constraint (ledger rows would keep their `event_id` as a historical reference only until cascade-nulled) and add an explicit `ON DELETE CASCADE` (or `SET NULL`) to `payment_attempt.event_promotion_checkout_id`. Deleting a ledger/payment trail outright is a financial-record retention question — flagging rather than picking one.
- **Status**: New finding, not fixed. Out of scope for the SEC-004 credential rotation that surfaced it — the rotation itself is verified working independently of this bug (see the rotation's own verification: the function reached real tables and hit real constraint errors, which only happens with genuine write privilege).
