...
| **Paystack** | Payments, card tokenization, refunds, (transfers — flag off), disputes | Payer email, amount, currency, card/MoMo details entered in Paystack UI, authorization codes; payout account details if transfers are enabled | Nigeria/Ghana | Paystack merchant agreement | `packages/services/src/payments/providers/paystackApi.ts`, `paystackTransfer.ts`, `api/paystack/webhook/route.ts` |
| **Hubtel** | SMS one-time codes | Phone numbers, the code | Ghana | Hubtel agreement | `packages/services/src/profile/otpProviders/hubtelOtpProvider.ts` |
| **Stripe** (not yet in use) | Card and wallet payments, refunds and disputes for markets that use it (GB, US, FR, DE drafts) — only once such a market is activated | Payer email, amount, currency, card details entered on Stripe's hosted page | Stripe's processing regions (confirm) | Stripe Services Agreement + DPA (confirm before activation) | `packages/services/src/payments/providers/stripeProvider.ts` |
| **Twilio** (not yet in use) | Phone one-time codes (Twilio Verify) for markets whose OTP provider is Twilio — only once such a market is activated | Phone number, the code (held by Twilio) | US (confirm) | Twilio Terms + DPA (confirm before activation) | `packages/services/src/profile/otpProviders/twilioVerifyProvider.ts` |
| **Open Exchange Rates** | Currency rates for display estimates | No personal data — Abonten only downloads a rate table | — | OXR terms | `packages/services/src/fx/exchangeRateCore.ts` |
