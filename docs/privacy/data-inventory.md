...
| Device push tokens (Expo), platform | `device_token` | Push notifications | Owner; service role | Pruned when Expo reports the device unregistered (send ticket or receipt) |
| Expo push ticket ids with the token they went to | `push_receipt` | Reading the delivery receipt to prune dead tokens | Service role | Deleted when the receipt is read, at most one day |
| Browser push subscriptions (push-service endpoint, encryption keys, user agent) | `web_push_subscription` | Browser notifications | Service role | Deleted on sign-out, opt-out, a 404/410 from the push service, beyond ten per person, and on account deletion |
| Email consent events (granted / withdrawn, where, when) | `notification_consent_event` | Record of consent for optional email (Act 843) | Service role | **POLICY DECISION REQUIRED** (legal item G3) |
