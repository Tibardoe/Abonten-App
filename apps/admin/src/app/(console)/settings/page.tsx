...
export default async function SettingsPage() {
  ...
  const canEditMatrix =
    ctx.permissions.includes("settings.manage") && stepUpFresh;
    ...
    <div className="space-y-6">
      ...
      {canManage && !stepUpFresh && (
        <p className="rounded-md border border-warning/40 bg-warning/10 p-2 text-sm">
          Managing admins needs a fresh re-authentication. Click{" "}
          <strong>Confirm identity</strong> above, then return here.
        </p>
      )}
      ...
      <section>
        ...
        {staff.status !== 200 || !staff.data ? (
          <EmptyState>
            {staff.message ?? "Couldn't load admin staff."}
          </EmptyState>
        ) : staff.data.length === 0 ? (
          <EmptyState>
            No admin accounts yet. Grant the first <code>super_admin</code> role
            directly in the database, then manage the rest here.
          </EmptyState>
        ) : (
          <Table>
            <thead>
      ...
      <section>
        ...
        {matrix.status === 200 && matrix.data ? (
          <RoleMatrixEditor matrix={matrix.data} canEdit={canEditMatrix} />
        ) : (
          <EmptyState>
            {matrix.message ?? "Couldn't load the role matrix."}
          </EmptyState>
        )}
        <p className="mt-2 text-xs text-muted-foreground">
          Live from <code>admin_role_permission</code>.{" "}
          <code>@abonten/core/adminPermissions</code> is now the seed + type
          source only; if a role&apos;s rows are ever wiped, that role falls
          back to its compiled defaults so no admin is locked out.
        </p>
      </section>
    </div>
  );
