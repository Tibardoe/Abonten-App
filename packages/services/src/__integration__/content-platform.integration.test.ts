...
// follows and the Story tray, telemetry de-duplication, moderation notices,
// and the campaign money path (activation, review, accrual, refund record,
// reconciliation).
...
describe("campaign money path", () => {
  it("activates only from a paid checkout, reviews, accrues and reconciles", async () => {
    const postId = await publishPost("spotlight");
    const created = await createContentCampaignCore(svc, organizer.id, {
      postId,
      presetId: 1,
      objective: "views",
    ...
    const { campaign, checkout } = must(created.data);
    expect(campaign.status).toBe("pending_payment");
    expect(checkout.status).toBe("pending");

    // The advertiser can never move their own campaign to active.
    const selfApprove = await svc.rpc("content_campaign_transition", {
    ...
    const { data: tx } = await svc
      ...
      .insert({
        ...
        paystack_reference: `IT-CAMPAIGN-${crypto.randomUUID()}`,
      } as never)
      .select("id")
      .single();
    ...
    const activate = () =>
      svc.rpc("content_campaign_activate_from_checkout", {
    ...
    expect((replay.data as { replayed?: boolean } | null)?.replayed).toBe(true);

    let { data: row } = await svc
      .from("content_campaign")
      .select("status, paid_minor, version")
      .eq("id", campaign.id)
      .single();
    expect(must(row).status).toBe("pending_review");
    expect(Number(must(row).paid_minor)).toBe(
      Math.round(checkout.totalPrice * 100),
    );

    const approve = await svc.rpc("content_campaign_transition", {
      p_campaign_id: campaign.id,
      p_to: "active",
      p_actor_id: viewer.id,
      p_actor_kind: "admin",
    ...
    // Pretend it has run for an hour, then accrue.
    await svc
      .from("content_campaign")
      .update({
        activated_at: new Date(Date.now() - 3_600_000).toISOString(),
      } as never)
      .eq("id", campaign.id);
    const accrued = await svc.rpc("content_campaign_accrue", {
      p_campaign_id: campaign.id,
    } as never);
    ...
    const cancel = await advertiserCampaignActionCore(svc, organizer.id, {
      campaignId: campaign.id,
      action: "cancel",
    });
    ...
    const amount = Number(refundable);
    expect(amount).toBeGreaterThan(0);
    expect(amount).toBeLessThanOrEqual(Number(must(row).paid_minor));
    ...
    expect((await record()).error).toBeNull();
    const second = await record();
    // Exactly once: a second request is a replay or refused, never a second refund.
    ...
    expect(second.error === null || second.error.code === "22023").toBe(true);
    ...
    expect((reconcile.data as { ledger_drift: number }).ledger_drift).toBe(0);

    await svc
      .from("content_campaign_ledger")
      .delete()
      .eq("campaign_id", campaign.id);
    await svc.from("transaction").delete().eq("id", txId);
  });
});
