...
// Post-auth body of the client-triggered verification — shared by the web
// action and `/api/mobile/payments/verify`. Caller supplies an already-
// authenticated client + the resolved userId; the ownership check on the
// payment_attempt is kept here. Same finalizePayment() the provider webhook
// calls, so whichever of the two wins the race does the real work.

export type VerifyPaymentCoreResult =
...
export async function verifyPaymentCore(
  supabase: SupabaseClient<Database>,
  userId: string,
  paymentAttemptId: string,
  deps: PaymentFulfillmentDeps,
): Promise<VerifyPaymentCoreResult> {
